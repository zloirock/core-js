...
- If the implementation for the `pure` version should differ from the global version, it should be added to [`packages/core-js-pure/modules`](./packages/core-js-pure/modules) directory. The rest of `core-js-pure` will be copied from `core-js` package.
