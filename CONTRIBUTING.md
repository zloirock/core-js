...
- Transpiler performance gates — complexity-class bounds over real packages and synthetic worst-case shapes. Measured by the clock, so they are not a part of the composite above and get a CI job of their own:
  ...
  ```
- Real-library end-to-end suite — both polyfill providers (`@core-js/babel-plugin` and `@core-js/unplugin`) + Babel down-compile to the ES5 floor across RxJS, three.js, CodeMirror, ECharts, TanStack Table, es-toolkit, ml-matrix, colorjs.io, the htmlparser2 stack and Planck.js (the last two built from their TypeScript sources wherever a package ships them). babel-plugin has no phase axis, so its injected set is the reference and each unplugin phase is snapshotted as a delta against it. The browser leg runs Chromium, Firefox and WebKit anywhere, and adds real IE11 on CI or on a machine that has it:
  ```sh
