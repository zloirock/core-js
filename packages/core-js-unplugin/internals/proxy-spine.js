...
import {
  ...
  dropChainMarkerWithoutOptional,
  foldRealmHopOverCarriedValue,
  foldUnbackedRealmHopsAbove,
  landingOverSubstitutedSpan,
  insideMemoClone,
...
// how far the navigation above a claim runs - the span a `delete` collapse would take. the MEMBER RUN
// is the span: a member continues it only while it reads THIS value as its object (reached as a
// computed KEY it is a sibling expression, not a continuation). the wrappers BETWEEN two hops - the
// chain marker, a TS cast, a source paren - are stepped THROUGH, because the comparison below peels
// the parent's object and the child alike. a seal bounds the SHORT-CIRCUIT, not this span: the `?.`
// it hides still decides whether the delete HAPPENS (`delete (nav.window?.self).slot`), so a span
// ending there would fold the run out from under the very guard that keeps it.
// where the span is deliberately WIDER than the canon chain end (`memberChainEndPath`): a carrier
// standing INSIDE it - a store, a sequence - hands the navigation on, while the canon follows one
// only at the ROOT step. the MUTATED-slot question is the other way round and rides the canon extent
// instead (`deletedRunCarriesMutatedRealmHop`): the fold lands its base UNDER those hops, so one
// standing anywhere on that extent decides for the whole run.
function deletedNavTopPath(metaPath) {
  let top = metaPath;
  // the span ends where the chain does - the root step carries no parent to continue onto
  for (let up = top.parentPath; up?.node; up = top.parentPath) {
    const upType = up.node.type;
    const topCore = unwrapRuntimeExpr(top.node);
    // ... and THROUGH the carrier standing inside the span: a store or a sequence hands the nav on,
    // and stopping at it anchored the delete question one member below the guard that decides it
    if (!SKIPPABLE_WRAPPER_TYPES.has(upType)
      && !(upType === 'MemberExpression' && unwrapRuntimeExpr(up.node.object) === topCore)
      && !chainValueCarrier(up.node, top.node)) break;
    top = up;
  }
  return top;
}
...
export default function createProxySpineChannel(ctx) {
  ...
  // the hop a MUTATING consumer keeps, asked of the whole chain the claim stands in: the shared
  // exception walks the RUN, and a claim's own node spells only the part of it above the probe
  function mutationKeptHopAt(metaPath) {
    return mutationGuardKeepingHop(metaPath.node, m => resolvePure(m, metaPath),
  ...
  function proxyHopKey(node, { allowOptional = false, metaPath = null } = {}) {
    ...
    const key = unwrapRuntimeExpr(node.property);
    // every SEQUENCE prefix inside the key is an effect of this hop, wherever it sits: nested levels
    // (`[(f++, (g++, 'window'))]`), and the ones a FOLD buries inside a concatenation or a template
    // (`[(eff(), 'se') + 'lf']`). they lift in source order and the hop then folds like its quiet
    // twin; an observable that is NOT such a prefix has nowhere to re-run, and there the hop stays
    const effects = [];
    let liftable = true;
    (function harvest(current) {
      if (!liftable || !current || typeof current.type !== 'string') return;
      if (current.type === 'SequenceExpression') {
        effects.push(...current.expressions.slice(0, -1));
        harvest(unwrapRuntimeExpr(current.expressions.at(-1)));
        return;
      }
      if (mayHaveSideEffects(current) && !current.expressions && !current.quasis
        && current.type !== 'BinaryExpression' && current.type !== 'ParenthesizedExpression') {
        liftable = false;
        return;
      }
      walkAstChildren(current, child => harvest(unwrapRuntimeExpr(child)));
    })(key);
    if (!liftable) return null;
    // ONE naming for every computed spelling of a hop: a string literal, a template, a key BOUND
    // to a constant string (`g[k]`, `const k = "self"`) all name the slot the dotted hop names, and
    // the canon key resolver folds them alike - asked here through the path, because a shape-by-shape
    // reading of the key node answered a NARROWER name than the same hop's claim, and the two legs
    // then folded one source two ways. an EFFECT-bearing key expression is not a name at all: the
    // peel above already lifted a sequence prefix out, and what is left has nowhere to re-run
    if (!metaPath) return null;
    const keyName = realmHopKeyName(node, { scope: metaPath.scope, adapter, path: metaPath },
      { allowSideEffectKeys: true });
    return typeof keyName === 'string' && isPristineProxyGlobal(adapter, keyName) ? { keyName, effects } : null;
  }
  ...
  function collapseProxyHopSpine(node, metaPath, { allowOptional = false, provenCallRoot = false } = {}) {
    ...
    const spineCtx = metaPath ? { scope: metaPath.scope, adapter, path: metaPath } : null;
    // does a `delete` FOLD this run - the operator naming a slot on the run itself? asked once for
    // every root kind: a dispatch standing between them consumes the run as its receiver, and
    // spelling the question per branch landed one source shape on two bindings (an identifier root
    // rode the hop, a call root the ponyfill root)
    const deleteFoldsThisRun = !!metaPath && !!deleteHostForClaim(metaPath, node, { forFold: true })
      && !dispatchConsumesRun({ path: metaPath, resolvePure: m => resolvePure(m, metaPath), aliasCtx: spineCtx });
    ...
    // plain value-read claims and the `delete` fold take it (a fold that names a slot ON this run
    // reaches it off the ROOT binding, the base the identifier spelling lands on; with a dispatch
    // between them the run is an ordinary read and lands like one) - the guard/memo channels
    ...
    if (cur?.type === 'CallExpression' && metaPath && !keptWrite && provenCallRoot) {
      ...
      const pure = proxyRunLandingPure({
        navNode: node,
        ctx: spineCtx,
        resolvePure: m => resolvePure(m, metaPath),
        rootName,
        deleteFold: deleteFoldsThisRun,
      });
      if (!pure) return null;
      return {
        entry: pure.entry,
    ...
    // what makes a kept WRITE the base of the navigation is an UNRESOLVABLE HOP inside the value it
    // stored - the same question the shared plan asks before keeping one: `(q = globalThis.window)`
    // hands on the environment probe and the run keeps the write, while a store whose value the
    // build can spell (`q = globalThis.self`) or one holding a plain realm read this build happens
    // not to spell (`es.global-this` excluded) leaves the run riding its own landing, the store
    // re-emitting ahead of it
    const writeStoreSpellable = !keptWrite || proxyStoreIsSpellable(keptWrite.right, resolveGlobalPolyfill)
      || (!!metaPath && !navHasUnresolvableProxyHop(keptWrite.right, m => resolvePure(m, metaPath), spineCtx));
    ...
    // ... and only where the NAME still reads the realm: a LOCAL binding wearing it (`function
    // f(globalThis) { delete globalThis.self.k }`) holds the user's own object, and collapsing onto
    // the ponyfill acts on core-js instead of on what the source named. through THE canon, which
    // answers name and alias alike with the shadow bail (and its position discipline) built in -
    // detection asks it of every claim, and the spine walks this root itself
    if (metaPath && !proxyGlobalRootName({ node: cur, scope: metaPath.scope, adapter, path: metaPath })) return null;
    // the run lands where the shared canon says - the deepest hop pure can back, the root only
    // when nothing above it is backed. the exception the canon names is a `delete` FOLDING this run
    // (`deleteFoldsThisRun` above): there the operator names a slot rather than reading a value, and
    // its base is the run's ROOT binding whatever the hops above spell
    const pure = metaPath
      ? proxyRunLandingPure({
        navNode: node,
        ctx: spineCtx,
      ...
      : resolveGlobalPolyfill(cur.name);
    ...
    // the STORED VALUE of a kept write is not what a `delete` names - the operator's base is the run
    // it navigates, the store keeps what the source put in it - so its own spelling comes from the
    // landing asked WITHOUT the fold's root rule (`delete (t = globalThis.self).window.Promise` ->
    // `delete (t = _self, _globalThis).Promise`)
    const valuePure = metaPath ? proxyRunLandingPure({
      navNode: node,
      ctx: spineCtx,
      resolvePure: m => resolvePure(m, metaPath),
      rootName: cur.name,
    ...
    return {
      entry: pure.entry,
      hintName: pure.hintName,
      valueEntry: valuePure?.entry ?? pure.entry,
      valueHintName: valuePure?.hintName ?? pure.hintName,
      effects,
      keyEffects,
      keptWrite,
      writeStoreSpellable,
  ...
  // does the RECEIVER carry a live probe guard? this leg's locked syntactic half - a `?.` whose
  // OWN key is the unresolvable hop keeps its guard here whatever the value below (the ut-family
  // locks) - union the canon's LIVE verdict. CALL-root only: off a name root the guarded-nav
  // route reading this verdict owes no probe. the DELETE fold asks the canon alone: the syntactic
  // half answers for a `?.` over a PROVEN root that canon calls dead, and one source then landed
  // its delete on two bindings
  ...
  function deleteHostForClaim(metaPath, node, { forFold = false, canonOnly = false } = {}) {
    // ... asked through the CARRIER as well: a store or a sequence INSIDE the deleted navigation is a
    // consumer the plain walk stops at, and every arm below reads this one verdict
    ...
    // ... and never a run an earlier emit already LOWERED into a guard's own null test: the source's
    // `?.` is that test now, the walks above cannot read it out of the scaffold, and the spine mark
    // keeps answering "deleted" for the span it was taken on
    if (runStandsInLoweredGuardTest(metaPath, unwrapRuntimeExpr)) return false;
    ...
    if (deletedRunCarriesMutatedRealmHop(metaPath, { adapter, unwrap: unwrapRuntimeExpr })) return false;
    if (!forFold) return true;
    // the FOLD-BASE arms ask the canon at the CLAIM (a syntactic own-key `?.` whose value is proven
    // defined is dead and must not demote the root fold to the leaf); the arm below asks the same
    // canon at the run's TOP, which is the anchor a fold reaching past this claim owes
    if (canonOnly) {
      return !deleteGuardKeepingHop(node, m => resolvePure(m, metaPath),
        { scope: metaPath.scope, adapter, path: metaPath });
    }
    ...
    // the top is handed on RAW: the climb ends on whatever the source wrote there - a chain
    // marker, a paren, a TS cast - and BOTH predicates below open on `unwrapRuntimeExpr` and
    // read the argument through nothing else, so a peel spelled here re-does theirs over a
    // SUBSET of the wrapper set and can only fall behind it
    const top = deletedNavTopPath(metaPath);
    // the canon and NOTHING beside it: the receiver predicate scopes itself to a call root because
    // the guarded-nav route reads it too, while the delete question is rooted wherever the source
    // rooted it - a name, or a carrier standing at one - so a union with it answered by the ROOT's
    // shape for a `?.` both agree is dead
    return !deleteGuardKeepingHop(top.node, m => resolvePure(m, metaPath),
      { scope: metaPath.scope, adapter, path: metaPath });
  }

  // the run a MUTATING consumer keeps WHOLE, asked where the FOLD reaches. a live `?.` in the run
  // decides whether the act LANDS, and a fold swallows the hops ABOVE the claim too, so the question
  // is anchored at the run's top - a seal, a TS wrapper or a further hop parks the deciding marker
  // exactly there, out of reach of the claim's own node. every value verdict beside a collapse
  // answers `undefined` where the source throws, the price only a READ may pay, so this outranks
  // them. `mutationKeptHopAt` is the RENDER's half of the same rule: it weighs the `?.` this claim's
  // own substitution would erase, which is the claim's node and nothing above it
  function mutatedRunKeepsItsGuards(metaPath) {
    return !!mutationGuardKeepingHop(unwrapRuntimeExpr(deletedNavTopPath(metaPath).node),
  ...
  function emitOwnOptionalGuardedClaim({ meta, metaPath, node, parent, kind, entry, hintName }) {
    ...
    // IDENTIFIER root the collapse family owns the whole nav and this arm stands down - except
    // across an SE-bearing sequence PREFIX, which that family's render lands INSIDE rather than
    // above: its root substitution spells the identifier in the prefix and the hops over it keep
    // their raw realm read, so standing down left `self` a native read nobody polyfills. a STORED
  ...
  function emitStaticOverGuardedNav({
    ...
    // the kept write as a comma prefix instead - the plain-swap tail renders that)
    {
      ...
      // ... and a probe the value canon calls defined still guards where a MUTATING consumer keeps
      // the `?.`: what that fold drops is not a value but the write itself
      // ... nor inside a guard TEST clone, where the render IS the read the test performs: the guard
      // built there answers for the run's UNBACKED hop, not for the source's `?.`, so a DEAD `?.`
      // standing this arm down handed the run to a fold that drops that read outright
      // (`(w = globalThis?.window.self.Array)?.from(x)` lost the test its `?.`-less twin spells)
      if (probe && !mutationKeptHopAt(metaPath) && !insideMemoClone(metaPath, probeTestClones)
        && !guardProbeUndefinable(probe,
  ...
  function emitRootGuardedNavCollapse(metaPath) {
    ...
    for (let up = metaPath.parentPath; up?.node; up = up.parentPath) {
      ...
      if ((upNode.type === 'ParenthesizedExpression' && upNode.expression === cursor.node)
        ...
        // ... and a SEQUENCE root rides along whatever its prefix does: an effect-free one is the
        // bare twin's spelling (`(0, globalThis)`), and an effect-bearing one is spelled by the
        // nested render's own test, which carries the probe hop's SOURCE slice - prefix included,
        // evaluated exactly once. stopping here left the run raw where its prefix-less twin folds
        || (upNode.type === 'SequenceExpression' && upNode.expressions.at(-1) === cursor.node)) {
        cursor = up;
        continue;
      }
    ...
    for (let at = climbed.length - 1; at >= 0; at--) {
      ...
      const plan = planProvenNavGuardCollapse({
        ...
        resolvePure: resolveHere,
        throughKeptAssign: true,
        // a sequence at the run's root is spelled by the nested test's source slice, so the plan
        // may own it - the render re-emits the prefix there and nowhere else
        allowSequenceRoot: true,
      });
      if (!plan || plan.kind !== 'nested' || !plan.leafPure) continue;
      if (plan.seqAroundPrefix?.length
      ...
      // ... and except where a `delete` names a slot on a run whose hops pure CAN back: the drop
      // would erase the very ponyfill that slot hangs off, so the guard render below spells it (the
      // shape the babel leg lands on the same source). a run carrying a hop pure cannot spell has no
      // such ponyfill to keep and drops as a read does
      const deletedBackedRun = deleteHostAboveChain(leafPath, leafPath.node, unwrapRuntimeExpr)
        && !navHasUnresolvableProxyHop(leafPath.node, resolveHere, aliasCtx);
      if (probeHopInValue(plan, probeHop) && !probeValueIsInvoked(leafPath) && !deletedBackedRun) {
        dropValueProbeNavHops(plan, climbed, metaPath);
        return false;
      }
  ...
  function dropValueProbeNavHops(plan, climbed, metaPath) {
    ...
    for (const path of outside) {
      const hop = proxyHopKey(path.node, { allowOptional: true, metaPath });
      if (!hop) break;
  ...
  function substituteProbeProxyRoot(root) {
    ...
    while (spine && typeof spine === 'object') {
      ...
      // WHAT such a call yields is the canon's answer (`singleReturnBodyExpression`, what the
      // value walk reads): descended by node shape, a BLOCK body shipped its raw read
      if (spine.type === 'CallExpression' && !spine.optional) {
        const callee = unwrapRuntimeExpr(spine.callee);
        const yielded = callee?.type === 'ArrowFunctionExpression' || callee?.type === 'FunctionExpression'
          ? singleReturnBodyExpression(callee.body) : null;
        if (yielded) {
          spine = unwrapRuntimeExpr(yielded);
          continue;
        }
      }
      break;
  ...
  function renderCollapseOverKeptWrite({
    ...
    // the write re-emits the run's own VALUE spelling, which the collapse resolved: the deepest hop
    // pure can back, asked without the `delete` fold's root rule - the operator names a slot, the
    // store keeps what the source put in it. rendering the CLAIM's entry here instead put the store
    // back on the root binding the landing had just declined
    const innerBase = innerKeptTail.reduce(respellKeptHop,
      identifier(injectPureImport(collapsed.valueEntry ?? collapsed.entry ?? entry,
        collapsed.valueHintName ?? collapsed.hintName ?? hintName)));
    // the STORED value's own effects, in source order: its sequence prefix ran before the value the
    // collapse spells, and the collapse's own effects ran inside it
    const storedEffects = [...valueEffects, ...collapsed.effects ?? []];
    ...
    if (navigated && collapsed.aliasRoot && collapsed.effects?.length && !outerEffects.length) {
      ...
      const inPlaceWrite = assignmentExpression('=',
        cloneNode(writeStep.left), withSideEffects(innerBase, storedEffects));
    ...
    const write = assignmentExpression('=', cloneNode(writeStep.left), withSideEffects(innerBase, storedEffects));
    ...
    // the wrappers standing ON the span go with it here too - one rule for where a substitution
    // lands, shared with the fold and the plain spine render
    const landed = landingOverSubstitutedSpan(target);
    landed.replaceWith(rendered);
    markSubtreeSkipped(skippedNodes, consumed);
  ...
  // eslint-disable-next-line max-statements -- one climb collecting every layer the collapse extends through
  function extendCollapseUpward(metaPath, { allowOptional, deadOptionalHop = null }) {
    ...
    // ... and effects a kept WRITE's own sequence runs INSIDE the value it stores: same climb, other
    // side of the write (`(w = (e++, globalThis)).self.k` runs `e++` before the value, not before
    // the store), so the re-emit owes them there
    const valueEffects = [];
    ...
    // does a BACKED hop stand above the write? an unbacked run over a store folds only THROUGH a
    // ponyfill (`.self.window` over the store folds; a lone `.window` reads the stored value)
    let outerBacked = false;
    for (let up = target.parentPath; up?.node; up = target.parentPath) {
      ...
      if (TRANSPARENT_EXPR_WRAPPER_TYPES.has(up.node.type) && up.node.expression === target.node) {
        ...
        // except a BARE paren, which asserts nothing about the value: the stored spelling reads the
        // same with or without it, and stopping there rendered `w = (globalThis)` unlike its
        // paren-less twin, which is exactly what the dialect discipline forbids
        if (assignmentHoldsValue(up) && up.node.type !== 'ParenthesizedExpression') break;
        target = up;
        continue;
      }
      ...
      if (up.node.type === 'SequenceExpression' && up.node.expressions.at(-1) === target.node) {
        ...
        if (!run || !proxyHopKey(run.member, { allowOptional: true, metaPath })) {
          // a sequence a kept WRITE holds is INSIDE the stored value, not a prefix of the run: its
          // effects stay where the source wrote them, and when the run goes on ABOVE the write the
          // write's own step owns the climb (`delete (w = (e++, globalThis)).self.k`). that step
          // decides it - the sequence only hands it the cursor
          // ... and only for a hop whose KEY is effect-free: an SE-bearing key rides the harvest
          // canon, and migrating it out of a store the other leg keeps whole is a spelling only
          // this leg would print (`(w = (e++, globalThis))[(e++, 'self')].k`)
          const held = peelParenAndTSParentPath(up);
          const overMember = peelParenAndTSParentPath(held ?? up)?.node;
          const seFreeHop = !proxyHopKey(overMember, { allowOptional: true, metaPath })?.effects?.length;
          if (!writeStep && held && seFreeHop
            && stepOverKeptWrite(held, up, { allowOptional, metaPath, proxyHopKey, deadOptionalHop })) {
            valueEffects.push(...up.node.expressions.slice(0, -1));
            target = up;
            continue;
          }
          break;
      ...
      // ... unless the shared vestigial verdict calls that `?.` DEAD: it reads a run the value canon
      // has already proven defined, so what sits below cannot revive it, and refusing it here left
      // the climb standing mid-run where the other leg folds it whole (`globalThis.self?.window.X`)
      const hop = proxyHopKey(up.node,
        { allowOptional: allowOptional || !keptTail.length || !!deadOptionalHop?.(up.node), metaPath });
      if (!hop) break;
      if (!resolveGlobalPolyfill(hop.keyName)) {
        ...
        keptTail.push({
          keyName: hop.keyName,
          keySe: hop.effects,
          keyNode: up.node.computed && !hop.effects.length ? up.node.property : null,
        });
      } else {
  ...
  function renderProxySpineCollapse({
    ...
    // ... and a nav standing INSIDE a guard TEST is the read that test performs: the test exists to
    // reproduce the source's own short-circuit, so an unbacked hop in it is load-bearing and the run
    // keeps its spelling over the deepest span pure can back (`null == (w = _self.window?.Array)`).
    // taken as an ordinary navigation the tail folded away and the test read an always-defined
    // ponyfill - the branch native short-circuits past. only where the tail IS that read though:
    // a PLAIN member navigates the run, so the probe under it is no value the source reads and folds
    // onto the landing exactly as it folds under a plain claim (`(s = globalThis.self.window.Number)
    // ?.isInteger(1)` stores `_self.Number`) - the landing canon's own reader rule, asked here with
    // the `?.` the SOURCE wrote rather than the value verdict over it: inside a test that verdict
    // reads the ponyfill the swap would land, which is the branch this test exists to keep
    const readerNavigatesTheTail = spineIsNavigated(target, keptTail,
      collapsed.keptWrite ?? writeStep, { deadOptional: false }).navigated;
    // ... and a hop whose KEY carries EFFECTS is one no fold may take: they would go with the hop and
    // this render has no slot to replay them in - the same boundary the landing canon draws. a QUIET
    // computed key is not that hop: it names the slot its dotted twin names, and refusing it by the
    // key's spelling kept a hop the very same source folds away when written with a dot
    if ((!readerNavigatesTheTail || keptTail.some(kept => kept.keySe.length))
      && keptTail.some(kept => !resolveGlobalPolyfill(kept.keyName))
      && insideMemoClone(metaPath, probeTestClones)) return false;
    ...
    if (writeStep) {
      // a store whose value ends on an UNBACKED hop hands on the raw host read (`(w = globalThis
      // .window)` stores what the environment's `window` holds), and an unbacked run over it with no
      // ponyfill to read THROUGH folds nowhere: re-reading the root there answered where native
      // throws off-window (`(w = globalThis.window).window.k`). through a backed hop the run folds
      // as it does over any ponyfill; the other leg's positional fold canon reads it the same way
      if (innerKeptTail.length && keptTail.length && !outerBacked) return false;
    ...
    if (navigated && collapsed.keptWrite && !collapsed.writeStoreSpellable) {
      ...
      if (consumer?.type === 'MemberExpression'
        ...
        // ... and never across a SEAL: the read above a paren is plain and observes the void the
        // store hands on (`delete ((w = gw.window)?.self?.window).k` throws off-window)
        consumer.optional = storeReadHopOptional(consumed)
          && optionalHeirAbove(consumer, unwrapRuntimeExpr(carried.node)) !== null;
      }
    ...
    const mutatedForcedFold = mutatedAbove && navHasUnresolvableProxyHop(
      metaPath.node.object, m => resolvePure(m, metaPath), { scope: metaPath.scope, adapter, path: metaPath });
    // ... and a `delete` lands the ROOT binding whatever the position says: the operator names a
    // slot, so the run under it is not a value this claim spells its own pure for - the canon the
    // owner set for this family, and the bytes the identifier twin prints. NEVER over a kept WRITE:
    // there this spelling is the STORE's value, and the store keeps what the source put in it
    // ... and only where the collapse HAS a root to land: a spine whose root this build cannot spell
    // carries no entry at all, and reading one off it injected `undefined` (a crash on the lowered
    // text the post pass sees).
    const deleteFolds = !!collapsed.entry && !storedUserAssignmentOf(metaPath)
      && deleteHostForClaim(metaPath, metaPath.node, { forFold: true });
    const rootSpelling = collapsed.aliasRoot && !sourceValuePosition
      ? cloneNode(collapsed.aliasRoot)
      : identifier((valuePosition || mutatedAbove) && !mutatedForcedFold && !deleteFolds
    ...
    // ... and the wrappers standing ON that span go with it: one rule for where a substitution lands,
    ...
    // ?.customZ`) - only where no guard was owed, which is what allowOptional marks. the verdict is
    // asked about what the render PUT IN the tree: a harvested prefix hands its tail on and a kept
    // tail respelled over the binding does not, and the verdict reads both off that node - handed
    // the bare binding instead, it could not recognise its own sequence and left `(e++, _globalThis)
    // ?.name` where the effect-free twin folds
    if (deadOptional) {
      deoptionalizeOverSubstituted({ metaPath: replaceAt, node: consumed, replacement, proxyRoot: true });
    }
  ...
  function emitProxyHopClaim({ meta, metaPath, node, entry, hintName }) {
    ...
    // ... and none of the value verdicts below reaches a `?.` a MUTATING consumer keeps: they answer
    // for a value, where a write or a delete performs an act instead - the shared exception says so
    // and the guard channel owns the shape
    const mutationGuard = !!mutationKeptHopAt(metaPath);
    const deadOwnOptional = node.optional && !mutationGuard
            && (probeIsKeptWrite || !spineHoldsKeptWrite(node.object))
    ...
    const probeTestUnbackedHop = insideMemoClone(metaPath, probeTestClones)
      && navHasUnresolvableProxyHop(node.object, m => resolvePure(m, metaPath), { scope: metaPath.scope, adapter, path: metaPath });
    ...
    const deadSpineOptionals = !navValueCanShortCircuit(node, m => resolvePure(m, metaPath),
      { scope: metaPath.scope, adapter, path: metaPath }, { throughChainAssign: true });
    let collapsed = sealedAbove || mutationGuard ? null
    ...
    // a root the collapse cannot spell (`window` - there is no `_window`; an entry this build does
    // not inject) leaves the SPINE with nothing to land, but the claim itself still rides the
    // deepest span pure can back: `window.self.Array` is `_self.Array`, the same value the source
    // reads, where standing down left a raw realm read in pure output. what stays raw is what
    // nothing can spell - the bare root, and a run with no backed hop under it (`window.window.X`)
    if (collapsed?.unbackedRoot) collapsed = null;
    ...
    if (!collapsed && !sealedAbove && !optionalAbove && !deleteHost && node.optional
      ...
      if (storeUndefinable) {
        markRewrite();
        // the read above inherits the `?.` only where nothing SEALS the hop away from it: a paren
        // between them ends the short-circuit, so the hop drops INSIDE the seal and the read above
        // stays plain, throwing on the void exactly where the source does
        if (optionalHeirAbove(above, node)) {
          above.optional = true;
          replaceNodeInTree(above, above.object, node.object);
        } else replaceNodeInTree(above.object, node, node.object);
  ...
  // THE question for a claim standing inside a DELETED realm run: what base does the delete land on?
  // one answer, because three channels used to reach it apart - a stand-down that hoped the root's own
  // claim would render, this redirect, and the spine render - and a form only one of them knew then
  // got two different answers (`delete (0, globalThis).self?.window` landed the claim's pure while its
  // prefix-less twin landed the root). the rule itself is the owner's: the delete names a SLOT, so the
  // run under it folds onto its ROOT binding and the carrier at that root - a sequence prefix, the
  // user's own store - re-emits ahead of it, deciding what RUNS and never what the delete lands on.
  // null where this claim is not part of such a run, or where the fold is refused (a live `?.` that
  // decides whether the delete happens, a store whose own value this claim spells)
  function deletedRunBase(metaPath, node, kind, hintName) {
    if (node.type !== 'MemberExpression' || kind !== 'global' || !POSSIBLE_GLOBAL_OBJECTS.has(hintName)) return null;
    if (!deleteHostForClaim(metaPath, node, { forFold: true })) return null;
    // a claim spelling a kept STORE's value keeps that value's own spelling: what the source stored
    // is the store's act, not the delete's (`delete (ntm = globalThis.self.window)?.X` stores the
    // probe run, and the differential's kept-value rows are what say so)
    if (storedUserAssignmentOf(metaPath)) return null;
    ...
    for (;;) {
      ...
      const stored = peelChainAssignment(root).value;
      if (!stored || stored === root) break;
      root = unwrapRuntimeExpr(peelReceiverSequenceTail(stored));
    }
    // a CALL root yields its realm through the shared plan - that verdict is the core's, read here
    // rather than re-derived; a name root answers by its own binding
    if (root?.type === 'CallExpression' || root?.type === 'OptionalCallExpression') {
      ...
      return callPlan?.verdict === 'fold-whole' ? callPlan.rootPure : null;
    }
    const rootName = root?.type === 'Identifier' ? asProxyGlobalName(root.name) : null;
    if (!rootName || rootName === hintName || !isPristineProxyGlobal(adapter, rootName)) return null;
    return resolveGlobalPolyfill(rootName) ?? null;
  }
  ...
  function emitStaticGlobalClaim({ meta, metaPath, node, kind, entry, hintName }) {
    ...
    // absorbing an optional member above is the pre-order SKIP's business, not a shape read here: a
    // `?.` member that resolved nothing renders nothing, and reading it by shape stood BOTH guard
    // routes down for one - this arm and the probe-guard arm below - leaving the short-circuit to
    // nobody (`delete ut()?.window?.self?.chrome` shipped as `delete (ut(), _self).chrome`)
    ...
    if (node.type === 'Identifier' && kind === 'global' && POSSIBLE_GLOBAL_OBJECTS.has(node.name)
      ...
      // ... asked through the CARRIER too: a store or a sequence inside the deleted navigation is a
      // consumer the plain walk stops at, and reading it as "no delete above" left the run spelled
      // where its carrier-less twin folds onto the root binding
      // ... and the FOLD half of it: a live `?.` over the environment probe decides whether the
      // delete happens, and every arm below this binding erases optionality on its word
      const rootDeleteHost = deleteHostForClaim(metaPath, node, { forFold: true });
      ...
      const proxyRun = plainProxyHopRunAbove(metaPath, proxyHopKey,
        { allowOptional: rootDeleteHost, deadOptionalHop });
      // ... and never under a TERMINAL run the source READS: that value IS the environment probe
      // (`globalThis.self.window`), so every hop stays spelled - the verdict the hop claim's own
      // stand-down takes. asked of the RUN, not of the walk's extent: a MUTATED terminal slot ends
      // the walk early, and reading that as "nothing rides above" folded the hops under a probe the
      // file happens to delete from elsewhere
      // where the run lands is the shared canon's question, asked of the RUN this claim navigates:
      // a backed hop over the root carries it (`globalThis.self.Array` -> `_self.Array`), and a
      // `delete` keeps the root binding by its own rule
      const runPure = proxyRun && proxyRunLandingPure({
        navNode: proxyRun.child,
        ctx: { scope: metaPath.scope, adapter, path: metaPath },
        resolvePure: m => resolvePure(m, metaPath),
        rootName: node.name,
        deleteFold: !!rootDeleteHost,
      });
      if (proxyRun && (rootDeleteHost || !unbackedTailRidesAbove(metaPath, m => resolvePure(m, metaPath)))
        && renderProxySpineCollapse({
          metaPath,
          collapsed: {
            entry: runPure?.entry ?? entry,
            hintName: runPure?.hintName ?? hintName,
            effects: [],
            keyEffects: [],
          },
          entry,
    ...
    // while the spine still stands - the tail fold below reads it
    const probeRunIsTheValue = probeRunIsTheSourceValue(metaPath,
      { resolvePure: m => resolvePure(m, metaPath), effects });
    // a `delete` reaches its slot off the run's ROOT binding - the one base the landing canon
    // leaves to a rule of its own; every other run stands where this claim already does, on the
    // deepest hop pure can back (`globalThis[(eff(), 'se') + 'lf'].Array` IS `(eff(), _self).Array`)
    const redirected = deletedRunBase(metaPath, node, kind, hintName);
    ...
    const substitutedAt = node.type === 'MemberExpression'
      && POSSIBLE_GLOBAL_OBJECTS.has(redirected?.hintName ?? hintName) && !probeRunIsTheValue
      ? foldRealmHopsAndLand({ target, consumed, replacement, metaPath }) : target;
  ...
  // the realm hops standing over a substituted binding fold away here, and the fold answers WHERE
  // that binding then lives: the `?.` verdict walks up from it, and from the pre-fold path it read a
  // hop the fold had taken out of the tree, leaving a guard over a binding that cannot be absent.
  // only over a PROVEN root - an opaque call root is undefinable by its own canon, so the guard the
  // source wrote over it stands (`dh().self.window?.name` keeps its `?.`)
  function foldRealmHopsAndLand({ target, consumed, replacement, metaPath }) {
    function resolveHere(meta) {
      return resolvePure(meta, metaPath);
    }
    const folded = foldUnbackedRealmHopsAbove(target, replacement, { adapter, resolvePure: resolveHere });
    return folded && proxyRunValueIsProven(consumed, resolveHere, { scope: metaPath.scope, adapter, path: metaPath })
      ? folded : target;
  }
  ...
  function deoptionalizeOverSubstituted({ metaPath, node, replacement, proxyRoot = false }) {
    ...
    function dropChainAt(chainPath) {
      if (!chainDropped && dropChainMarkerWithoutOptional(chainPath)) chainDropped = true;
    }
    ...
    if (node.optional) dropChainAt(metaPath.parentPath);
  }
