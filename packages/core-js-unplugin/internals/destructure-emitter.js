...
export function createDestructureEmitter({
  ...
  // preceding extract statements for one declaration REGISTER in two phases (eagerly at prop
  // dispatch; at the receiver-copy drain once composed receiver text exists) - FIFO point-
  // inserts would order them by phase, not by source. entries carrying `order` (the
  // dispatching prop's position) buffer here and flush sorted in `applyDestructuringTransforms`,
  // so extraction statements follow the props' source order like the babel twin (whose
  // per-prop AST inserts are inherently source-ordered)
  const pendingPrecedingExtracts = [];

  function emitPrecedingDeclStatement({ declPath, insertPos, text, order = null }) {
    if (isBodylessStatementBody(declPath)) {
      bodylessBlockWrapEntry(declPath.node).before.push({ order, text });
    } else if (order === null) {
      transforms.insert(insertPos, text);
    } else {
      pendingPrecedingExtracts.push({ insertPos, order, text });
    }
  }
  ...
  // ObjectPatterns `handleDestructuringPure` fully-consumed as static synth-swaps whose receiver SURVIVES (a
  // buried effect must run): collapseProxyHopRoot must COLLAPSE that residual receiver's proxy hop, not defer it
  // as an ObjectPattern source (which strands a raw `_globalThis.self.X`). marked on the pattern visit, BEFORE
  // the visitor descends into the init - the PATTERN is the stable node the defer keys on, so the receiver's own
  ...
  function plannedAssignRewrite(assignNode, scope, path) {
    ...
    const result = withTrackedUnusedNames(unusedIds, () => {
      // the cascade keeps a FULL-consume SE-bearing receiver tail itself (`emitReceiverTail`
      // emits it as a standalone statement) - neutralize the plan-level discard-SE harvest
      // there so the setup isn't ALSO re-run by an extraction prefix. a PARTIAL consume
      // (surviving residual: a rest sibling / a non-consumed prop) keeps the harvest - the
      // rebuilt residual init is its single emission slot (the verbatim SE-bearing tail,
      // `(se(), _self)`), and the cascade's tail skip would otherwise drop the setup outright.
      // planning runs INSIDE the tracked window: it may mint `_unused` sentinel names that
      // need the explicit declarations
  ...
  function resolveSinkTailSrc(decl, receiverTail, refSplices) {
    ...
    // a tail hiding a nested SE (`[(eff(), globalThis)]` - the effect sits below the top-level
    // sequence peel, inside an array element) keeps its whole text in the sink: the bare pure
    // binding would silently drop the buried effect (the for-init sink is the init's single
    // re-emission slot; nothing lifts separately here). the flag was decided at seed time,
    // which also left the tail subtree UNskipped - the natural visitor owns the buried root's
    // substitution and composes it into the re-embedded slot by needle
    const receiverPure = !isAliasedIdentifier && !decl.sinkKeepsVerbatimTail && decl.receiver
  ...
  function flattenDeclarationPath(declPath, scope) {
    ...
    // for-init SE-sink tails that still hide a nested SE (`(eff(), [(eff2(), globalThis)])` -
    // the second effect sits below the top-level sequence peel) re-emit verbatim, so their
    // subtree must stay live for the natural visitor: decided HERE (before the skip seed runs)
    // and consumed by both the seed gate and `resolveSinkTailSrc`
    if (isForInit) {
      for (let i = 0; i < perDecl.length; i++) {
  ...
  function tryFlattenLiftedProxyHopHost(assignNode, operandNode, scope, path) {
    ...
    // an anchor-less plan is admissible only when the render provably discards the init: no
    // residual needs re-anchoring, the operand collapses to the extraction assigns (a ctor-alias
    // / pure-static host)
    if (!plan || (!plan.anchor && !planDiscardsInit(plan))) return;
  ...
  function polyfillInitGlobals(info) {
    ...
    const substituted = substituteProxyGlobalRoot({ node: initNode, src: info.initSrc, baseStart: info.initStart, aliasCtx });
    if (substituted !== null) return substituted;
    // every canon bailed (a mutated hop under a sealed chain) but the ROOT identifier's own
    // rewrite is SUPPRESSED (this init is re-emitted by the extraction): substitute just the
    // root in place - raw hops stay, the AST emitter's standdown spelling (`_entries((
    // _globalThis.window?.self).Object)` under a mutated-self deopt). a LIVE root keeps the
    // verbatim init: its visitor rewrite composes in by needle, and a second substitution
    // here would break that needle. this is the FINAL consumer - no later canon to defer to
    const proxyRoot = findProxyGlobal(peelNestedSequenceExpressions(initNode).tail ?? initNode, aliasCtx);
    if (proxyRoot?.type === 'Identifier' && skippedNodes.has(proxyRoot)) {
      const rootPure = resolveGlobalPolyfill(proxyRoot.name);
      if (rootPure) {
        const off = proxyRoot.start - info.initStart;
        return info.initSrc.slice(0, off)
          + injectPureImport(rootPure.entry, rootPure.hintName)
          + info.initSrc.slice(off + proxyRoot.name.length);
      }
    }
    return null;
  }
  ...
  function emitPolyfilled(info, parts, ctx, preDrainedSplices = null) {
    ...
    // a rest element is never a polyfill entry's prop, so it always lands in `remaining` - the
    // full-consume question is `remaining.length === 0` alone, and spelling `!hasRest` beside it
    // made the two full-consume gates read as different tests
    const isFullConsume = remaining.length === 0 && !hasInstance;
    ...
    const willLiftSE = isFullConsume && initSrc && mayHaveSideEffects(info.initNode);
    // a SEALED probe init on a FULL consume (`{ of } = (globalThis.window?.self).Array` - no
    // residual re-reads the init): the collapse drops the read the source performs on the
    // sealed VALUE. re-emit it as a THROW probe (carrying the nav's key SE) on the LAST
    ...
    if (isFullConsume && entries.length && info.initNode) {
      ...
      if (throwProbe) {
        ...
        info.probeSeNodes = [...info.probeSeNodes ?? [], ...throwProbe.keySeExprs];
      } else {
    ...
    if (initStart !== undefined && info.initNode) {
      ...
      const initUnread = isFullConsume;
      ...
      const harvested = willLiftSE && shouldDropRescueReceiver(peeledLiftTail ?? info.initNode)
      ...
      // SE nodes the sealed throw probe already carries must not lift a second run; when the
      // probe covers EVERY harvested effect the lift is fully subsumed - emit nothing
      const discardedSe = info.probeSeNodes ? harvested?.filter(node => !info.probeSeNodes.includes(node)) : harvested;
      if (harvested?.length && !discardedSe?.length) {
        liftParts = [];
        liftTailDead = true;
      } else if (discardedSe?.length) {
    ...
    // a receiver whose effect is buried in a proxy-hop MEMBER (`globalThis.self[(c++,'window')].Array`) is
    // FULLY collapsed by collapseProxyHopRoot (its pattern is in `consumedStaticResidualPatterns`): it does a
    // multi-hop drop + SE harvest that the single-hop text-delete below cannot, and running BOTH queues two
    // overlapping transforms on the re-emitted residual (`composedRangeSrc` then nests a stale `.self` needle ->
    // compose crash). let collapseProxyHopRoot own those; this single-hop delete keeps the non-effecting static
    // tails (`globalThis.self.Array`) it already handled, which collapseProxyHopRoot never marks
    // collapseProxyHopRoot fully owns (multi-hop drop + SE harvest) a marked proxy / ALIAS receiver - including a
    // mixed static+SE hop and a proxy operand inside a logical: pass aliasCtx so the predicate descends logicals
    // AND requires a real proxy / alias root at the member, i.e. exactly the set collapseProxyHopRoot collapses.
    ...
    const initIsUsed = remaining.length > 0 || hasInstance;
    ...
    const needsMemo = hasInstance && !initIsBareRef && (entries.length > 1 || remaining.length > 0);
    ...
    // a lift fully subsumed by the sealed throw probe (empty parts) emits nothing
    if (willLiftSE && initTransformed) {
  ...
  function flushPendingFlatten() {
    for (const entry of pendingFlatten) {
      ...
      // a for-init slot is never at a statement-list position, so nothing can fuse into it from the
      // left - the same rule the sibling whole-declaration overwrite in `applyDestructuringTransforms`
      // states. spelled here too, so the two sites cannot drift into different answers
      transforms.add(start, end, isForInit ? replacement : guardOverwriteLeftFusion(start, replacement, declPath),
        null, { rewrittenRanges: slotRanges });
    }
  ...
  // walk in original declarator order, flushing buffered preserved declarators when crossing
  // an extracted slot.
  ...
  function skipPatternExceptResidual(root, residualTargets, keepNodes = null) {
    const rescueRanges = [
      ...residualTargets.map(t => ({ start: t.srcStart, end: t.srcEnd })),
      // consumed subtrees whose source re-emits verbatim inside an extraction (a pattern-valued
      // symbol extraction) keep their inner rewrites queued for the compose splice
      ...(keepNodes ?? []).map(node => ({ start: node.start, end: node.end })),
    ];
  ...
  function seedSkippedForExtractedDeclarators(declaration, perDecl, keyCtx) {
    ...
    for (let i = 0; i < perDecl.length; i++) {
      ...
      skipPatternExceptResidual(decl.id, residualTargets, perDecl[i].keepVisibleNodes);
    ...
    // the exact negation of the first loop's gate: an ANCHORED zero-extraction slot is a flatten
    // host, already skip-seeded above - walking it here too queues a competing receiver
    // substitution on the residual default the first loop deliberately left rescued
    if (flattenedReceivers.size) {
      for (let i = 0; i < perDecl.length; i++) {
        if (!perDecl[i].extractions.length && !perDecl[i].anchored) {
  ...
  // the PLAN-side spelling of the emit's own discard test (`!preservedOuter.length` in
  // `rewriteDeclarator`): a prop leaves no residual when it is CONSUMED or ANCHORED - the anchored
  // remainder moves onto the ctor binding - and a rest element always emits a sentinel back into
  // the residual. every pre-pass that speculates on a dead init asks THIS, so plan and emit cannot
  // answer differently: asking the stricter `every kind === 'consumed'` left an init the emit
  // discarded anyway with no pre-pass having said so, and a destructure host buried in that init's
  // SE prefix lost its flatten to the weaker native-wins default injection
  function planDiscardsInit(plan) {
    return !!plan && plan.outerProps.every(o => o.kind === 'consumed' || o.kind === 'anchored')
  ...
  // its own)? the flatten then OWNS the prop's emission: the plan's existence alone makes
  // the flatten fireable from ANY sibling's dispatch (even an unresolved leaf), so a
  // per-prop route firing beside the whole-decl rebuild double-consumes the prop
  // (standalone extraction + the plan's own synth) and crashes the needle composition.
  // consulted EAGERLY at dispatch (the plan is cached) so prop order cannot decide ownership
  ...
  // source for one anchored-residual prop. a computed `Symbol.X` key here would leak NATIVE
  ...
  function emitOuterExtraction(e, scope, getReceiverSrc) {
    if (e.anchoredResidual) {
      ...
      const innerSrc = `{ ${ e.anchoredResidual.residualProps.map(p => anchoredResidualPropSrc(p, scope)).join(', ') } }`;
    ...
    if (e.synth === 'symbol-iterator') {
      const binding = injectPureImport(SYMBOL_ITERATOR_PURE_RESULT.entry, SYMBOL_ITERATOR_PURE_RESULT.hintName);
      // a pattern-valued extraction destructures the helper result; the pattern text re-emits
  ...
  function rewriteDeclarator(declarator, scope, usePath = null) {
    ...
    // for defined navs and shapes the shared plan cannot guard (callers keep their renders)
    function probedAnchorGuardSrc() {
      ...
      if (embedded && plan.discardSe) {
        const kept = plan.discardSe.filter(node => node !== embedded
          && !(node.start >= embedded.start && node.end <= embedded.end)
          && !(embedded.start >= node.start && embedded.end <= node.end));
        discardSeSrc = kept.length ? kept.map(node => nodeSrc(node)).join(', ') : null;
      }
      return navGuard.src;
    }
    function anchoredInitSrc() {
      ...
      if (guardSrc) return `(${ guardSrc }).${ plan.anchor }`;
      if (plan.anchorPure) return injectPureImport(plan.anchorPure.entry, plan.anchorPure.hintName);
    ...
    function synthReceiverSrc() {
      ...
      if (plan.anchor) return anchoredInitSrc();
      ...
      const pure = !isAliasedIdentifier && plan.receiver ? resolveGlobalPolyfill(plan.receiver) : null;
    ...
    function receiverEmitSrc() {
      ...
      // so its inner content (`globalThis`) still earns its own substitution. a FALLBACK tail
      // (all-proxy ternary / logical - the plan collapsed it only to RESOLVE the receiver) also
      // stays verbatim: the surviving residual must keep the branching read (operands earn their
      // own proxy substitutions), matching the babel residual render
      const tailIsFallback = tail?.type === 'ConditionalExpression' || tail?.type === 'LogicalExpression';
      // a PROBED nav init must not whole-swap to the always-defined receiver binding (the
      // probe's throw would vanish) - fall through to the shared resolver, whose guard
      // spelling reproduces the source read
      const receiverPure = isAliasedIdentifier || plan.discardSe || tailIsFallback || plan.probedNav
        ? null : resolveGlobalPolyfill(plan.receiver);
        ...
        ? initElementSwapped
          ? spliceInRange(nodeSrc(tail), tail.start, [
            ...(plan.consumedLevelStrips ?? []).flatMap(l => l.wrapper.start < tail.start ? [] : [
              ...
              { start: l.array.end, end: l.wrapper.end, content: '' },
            ].filter(sp => sp.start < sp.end)),
            {
              start: plan.initElement.start, end: plan.initElement.end,
              content: injectPureImport(receiverPure.entry, receiverPure.hintName),
            },
          ])
    ...
    for (let i = 0; i < plan.outerProps.length; i++) {
      // a prop CLAIMED by another pipeline (`skippedNodes` - e.g. the array-wrapped static
      // extract queued its own preceding decl + rest sentinel, then marked the prop) renders
      // VERBATIM regardless of the cached plan: the residual target re-anchors the claimer's
      // queued transforms into the rebuild. re-running the plan's consume would double-
      // sentinel the prop and crash the queue's needle substitution. render-time is the one
      // point where every pipeline's claims are complete (plans may be cached pre-claim).
      // a claimer that owns only the BINDING marks the prop's VALUE subtree, not the prop
      // (the instance / symbol-key extract keeps the key visible for its own rewrite), so a
      // value-marked prop is claimed the same way
      const claimedProp = plan.pattern.properties[i];
      const outer = claimedProp && (skippedNodes.has(claimedProp) || (claimedProp.value && skippedNodes.has(claimedProp.value)))
    ...
    if (!preservedOuter.length) {
      // an UNDEFINABLE probe nav under a FULL consume: the discarded init's read must still
      // throw where the source does - ride the guarded first-key read ahead of the FIRST
      // extraction's binding, once per pattern (native throws before any prop read). covers
      // the anchored hop AND the flat / array-wrapped consumed shapes via the shared key
      const fullConsumeProbeKey = probedNavProbeKey(plan);
      if (fullConsumeProbeKey && extractions.length) {
        const guardSrc = probedAnchorGuardSrc();
        if (guardSrc) {
      ...
      // extraction's binding (a synth extraction's receiver arg is SE-peeled, so the harvest
      // is always still unconsumed here)
      ...
      return {
        ...
        discardSeNode: plan.discardSe ?? null, anchored: !!plan.anchor, keepVisibleNodes,
      };
    }
  ...
  function canFullyConsumeProxyDeclarator(d, scope, usePath = null) {
    return planDiscardsInit(planDeclarator(d, scope, usePath));
  }
  ...
  // the fallback-receiver per-branch dispatch, for BOTH the top-level destructure path
  // (`const {from} = cond ? Array : Set`, assignment-target) and the parameter path. the receiver
  // lives either on a destructure wrapper slot (`{p} = R`) or as the IIFE call-arg
  // (`(({p}) => body)(R)`) - both shapes unified by `resolveFallbackReceiver`. per-branch
  // synth-swap rewrites each conditional / logical branch to its own polyfill object literal, so a
  // non-Identifier receiver (`cond ? Array : Set`) still gets array narrowing on each branch
  ...
  function tryRegisterIifeArgInstanceSynthPure(pureResult, metaPath, propNode) {
    ...
    // the ARGUMENT's own path feeds the receiver typing, and it comes from the SAME resolver that
    // located `receiver` - the callee-wrapper climb and the position both live there. deriving the
    // position here instead, by identity over top-level `arguments`, lost every receiver the
    // resolver had expanded out of an inline-array spread and fell back to the generic dispatcher
  ...
  function collapseRetainedProxyDefault(receiver, aliasCtx = null) {
    ...
    // no `allowSideEffectKeys` here, unlike the babel twin: an SE-bearing hop key is PRE-ROUTED
    // above through the shared root-collapse, so by this line the SE-key complement is already
    // gone. the twin has no such pre-route and has to take the flag to reach the same shapes -
    // one rule, two spellings forced by where each emitter renders it
  ...
  function tryHandleSideEffectKeyDeclaration(meta, metaPath, propNode, pureRaw) {
    ...
    // Maybe-dispatch instance helper returns a dispatcher, not the native method
    const patternValue = !localId && isSymbolIteratorPatternProp(propNode)
      && isSourcedSymbolIteratorMeta(meta) ? propNode.value : null;
    if (!localId && !patternValue) return false;
    const declPath = walkUpNestedDestructureToDeclaration(metaPath.parentPath);
    const declaration = declPath?.node;
    ...
    // memo evaluates exactly where the init did, so every buried effect runs once in source
    // order, whatever the expression shape (this emitter renders it via `memoRouted`)
    const receiverPlan = resolveDestructureReceiverPlan(metaPath, {
      allowSeFreeSingleRead: true, adapter: estreeAdapter, resolvePureGlobal: resolveGlobalPolyfill,
    });
    ...
    // template, an unmatched hop) is unextractable - leave native. the planner's `receiverIsSafe`
    ...
    // the flatten slot is resolved HERE, ahead of the commit block below. every other bail in this
    // function sits ahead of the commits; resolving after them left one path where the prop's binding
    // was already renamed to a `_unused` sentinel with nothing declaring it, the import was injected
    // unused, and the re-entry guard reported the prop handled so no other route could rescue it
    ...
    if (isFlattenClaimed && !flatten) return false;
...
`, order: declarator.start }),
  ...
  function tryNestedAssignmentInstanceOverwrite(meta, metaPath, propNode, pureRaw) {
    ...
    const receiverNode = resolveNestedReceiverNode(metaPath, { allowSePeeledFragment: true });
  ...
  function handleParameterDestructurePure(meta, metaPath, propNode, pureRaw) {
    ...
    if (!receiver) {
      ...
      const keyHasSideEffect = computedKeyHasSideEffects(propNode);
      ...
      // host's kept init has no other rendering channel)
      if (keyHasSideEffect) handledSideEffectKeyProps.add(propNode);
      return;
    }
    const binding = injectPureImport(pureResult.entry, pureResult.hintName);
  ...
  function tryBodyExtractFromParamDestructurePure({ propPath, propNode, binding, objectPattern, entry }) {
    ...
    if (restSibling) {
      ...
      // `undefined: _unused`, leaking the consumed prop into rest)
      transforms.add(propNode.start, propNode.end, `${ flattenKeySrc(propNode) }: ${ injector.generateUnusedName() }`);
  ...
  // pure resolution for a destructure prop meta, symbol-provenance aware: the babel twin
  // synthesizes the instance kind BEFORE dispatching the prop into its destructure pipeline,
  // so every kind-driven gate there (nested-instance routing, the SE-key / instance extract,
  // the assignment overwrite) sees `[Symbol.iterator]` as an instance member. a raw
  // `resolvePure` at those gates left the symbol binding native in every wrapped host
  function resolveDestructurePure(meta, pureRaw) {
    return isSourcedSymbolIteratorMeta(meta) ? SYMBOL_ITERATOR_PURE_RESULT : pureRaw;
  }

  // the destructure prop's host is a `catch` binding pattern - the one destructure host with no
  // declarator to rewrite, so every route that needs statement machinery has to decline on it
  function isCatchClauseHost(metaPath) {
    return metaPath.parentPath?.parentPath?.node?.type === 'CatchClause';
  }
  ...
  function resolveDestructureEntry({ isSymbolIterator, isSymbolKeyPassthrough, pureResult }) {
    if (isSymbolIterator) pureResult = SYMBOL_ITERATOR_PURE_RESULT;
    else if (isSymbolKeyPassthrough) return { kind: 'symbol-key', binding: null };
  ...
  function tryExtractArrayWrappedStaticPure(meta, metaPath, propNode, pureRaw) {
    const pureResult = pureRaw;
    if (!pureResult) return false;
    ...
    // rename the consumed key to `_unused` via the canonical key render (bracket form for
    // computed, quotes for literal keys): the residual array destructure keeps its shape
  ...
  function handleDestructuringPure(meta, metaPath, propNode) {
    ...
    // ONE resolve per dispatch, threaded into every gate below - the same shape the AST emitter
    // uses. each gate used to ask the resolver again for the identical `(meta, metaPath)` pair, up
    // to five times on the nested / SE-key / param routes, and a resolver that is not stable across
    // a dispatch would have let those gates disagree with each other
    const pureRaw = resolvePure(meta, metaPath);
    if (meta.fromFallback || isSourcedSymbolIteratorMeta(meta) || pureRaw) {
    ...
    if (computedKeyHasSideEffects(propNode)) {
      if (tryHandleSideEffectKeyDeclaration(meta, metaPath, propNode, pureRaw)) return;
      ...
      if (tryNestedAssignmentInstanceOverwrite(meta, metaPath, propNode, pureRaw)) return;
    ...
    // instance / pending / per-branch routes below never race the whole-decl rebuild. a
    // fromFallback meta stays owned too: the plan exists only when the fallback init is
    // wholly discardable (an all-proxy ternary - the flatten/extract canon), while a
    // diverging receiver bails the plan and keeps the per-branch mirror below
    if (isSourcedSymbolIteratorMeta(meta) && flattenPlanConsumesProp(metaPath, propNode)
        && tryFlattenNestedProxy(metaPath)) return;
    ...
    if (tryExtractArrayWrappedStaticPure(meta, metaPath, propNode, pureRaw)) return;
    ...
    // its extraction destructures the helper result, which the standalone channel below
    // cannot host (it collects bare-Identifier bindings only)
    if ((outerHost?.node?.type === 'Property' || directParent?.node?.type === 'ArrayPattern'
        || directParent?.node !== outerHost?.node
        || (isSourcedSymbolIteratorMeta(meta) && isSymbolIteratorPatternProp(propNode)))
        && resolveDestructurePure(meta, pureRaw)?.kind === 'instance') {
    ...
    const { value } = propNode;
    // bail BEFORE seeding `skippedNodes` below: a non-binding value fails
    // `propBindingIdentifier` and exits here. seeding the `Symbol.iterator` key earlier would
    // suppress the standalone Symbol-Identifier visitor from emitting `_Symbol$iterator`,
    // dropping the polyfill entirely. EXCEPTIONS that proceed, re-emitting the pattern LHS over
    // the helper result: a symbol prop-DEFAULT over a pattern (`[Symbol.iterator]: {...} = fb`,
    // default-guarded), and a bare symbol pattern on a CATCH host (the SE-key pipeline upstream
    // declines catch - it needs a declaration; this channel's catch relocation hosts it)
    const symbolPatternDefault = value?.type === 'AssignmentPattern' && value.left?.type === 'ObjectPattern'
      && isSourcedSymbolIteratorMeta(meta);
    const symbolPatternCatch = isSymbolIteratorPatternProp(propNode)
      && isSourcedSymbolIteratorMeta(meta) && isCatchClauseHost(metaPath);
    if (value && !propBindingIdentifier(value) && !symbolPatternDefault && !symbolPatternCatch) return;
    ...
    const pureResult = isSymbolIterator ? null : pureRaw;
    ...
    const isSymbolKeyPassthrough = !pureResult && !isSymbolIterator && patternHasRest
      ...
      && isCatchClauseHost(metaPath);
    if (!pureResult && !isSymbolIterator && !isSymbolKeyPassthrough) return;

    const objectPattern = metaPath.parent;
    const isDefault = value?.type === 'AssignmentPattern';
    // a prop-level default over a pattern-valued symbol prop (`[Symbol.iterator]: {...} = fb`):
    // the LHS re-emits the pattern source, destructuring the default-guarded helper result -
    // matching the babel ternary emission. inner rewrites (a default's instance call) splice in
    ...
    if (initNode && !isAssignment && !isCatchClause
      ...
      // hosts drop / collapse the tail
      && !classifyVariableDeclarationHost({
        declaration: declPath.node, declarationParent: declPath.parentPath?.node,
      }).isBodyless) {
      // a receiver whose effect is BURIED IN THE MEMBER (the hop KEY `globalThis[(c++,'self')].Array`, not a
      // liftable top-level prefix `(se(), X)`) survives as a residual (leading statement, or a for-init SINK
      // declarator) and is NOT a droppable dead tail. mark the PATTERN so collapseProxyHopRoot collapses the
      // receiver's proxy hop (matching babel) instead of deferring it raw off the pure root. peel transparent
      // wrappers + the SEQUENCE TAIL first so a lifted prefix (`(se(), globalThis.self.Array)`, member is
      // effect-free -> lifted standalone) does NOT mark - only an effect in the member itself. marking the
      // pattern (not the receiver member, whose identity shifts under wrappers) lets the hop-climb do all the
      // proxy / alias detection. for-init reaches here too - the AST still carries the `{ ... }` pattern.
      // a pure-CTOR leaf (`globalThis[(c++,'self')].Symbol` - the WHOLE `Symbol` is a pure global) is EXCLUDED:
      ...
      // under a sink declarator, so its tail stays and must keep substituting (unmarked)
      if (declPath.parentPath?.node?.type !== 'ForStatement') {
    ...
    // an assignment-destructure (`({from} = globalThis[(c++,'self')].Array)`) re-emits the consumed receiver as
    // a residual statement just like the VariableDeclaration above, so mark its pattern the same way (the block
    // above is declaration-only; no bodyless / for-init host concept applies to an expression statement)
    if (initNode && isAssignment && !isCatchClause
      && pending.entries.length === objectPattern?.properties?.length
      && pending.entries.every(e => e.kind === 'static')) {
      markConsumedStaticProxyResidual(initNode, objectPattern);
  ...
  // a consumed proxy receiver kept as a RESIDUAL whose effect is buried in a hop MEMBER: a direct member
  // (`globalThis[(c++,'self')].Array`) OR a logical OPERAND of one (`(globalThis[(c++,'self')].Array) || Array`,
  // the residual keeps the whole logical for the effect). a pure-CTOR leaf (`...Symbol`, fully-consumed by a
  // static-method/symbol synth-swap) is INCLUDED: the shared resolveReceiverSource now harvests the hop-key SE
  // ahead of the pure-ctor collapse (`(c++, _Symbol)`), so marking it collapses the residual instead of leaking
  function buriesEffectInProxyHopMember(node, aliasCtx) {
    const tail = unwrapInitForResolution(node);
    if (tail?.type === 'LogicalExpression') {
      return buriesEffectInProxyHopMember(tail.left, aliasCtx) || buriesEffectInProxyHopMember(tail.right, aliasCtx);
    }
    const leaf = tail?.type === 'MemberExpression' || tail?.type === 'OptionalMemberExpression'
      ? memberKeyName(tail) : null;
    // when aliasCtx is given (the suppression gate) ALSO require a real proxy / alias root at the member - that is
    // exactly when collapseProxyHopRoot collapses it, so the single-hop default must stand down. the marking
    // caller omits aliasCtx (over-marking is harmless: collapseProxyHopRoot self-gates on the root)
    return !!(leaf && mayHaveSideEffects(tail) && (aliasCtx === undefined || findProxyGlobal(tail, aliasCtx)));
  }

  // mark a destructure PATTERN whose consumed proxy receiver buries an effect in a hop MEMBER, so
  // collapseProxyHopRoot collapses it in place instead of deferring it raw off the pure root
  function markConsumedStaticProxyResidual(initNode, objectPattern) {
  ...
  function triggerConsumedReceiverHopCollapse(declaratorPath, isAssignment) {
    ...
    if (rootPath?.node && !skippedNodes.has(rootPath.node)) collapseProxyHopRoot(rootPath);
  }

  // a CALL / IIFE-rooted proxy chain (`const f = () => globalThis; {iterator} = (f()).self.Symbol`) is neither a
  // bare proxy root for collapseProxyHopRoot NOR a bare hop for collapseRetainedProxyDefault (both need a bare
  ...
  function triggerCallRootReceiverCollapse(declaratorPath, isAssignment) {
    ...
    // the helper's own `VariableDeclarator` test already answers no for an assignment host
    if (discardedProxyReceiverDropsNav(declaratorPath?.node, initPath.scope, initPath)) return;
  ...
  function substituteProxyGlobalRoot({
    ...
    // an alias-rooted READ nav that short-circuits over an undefinable probe (a live `?.` or a
    // sealed paren layer) renders the kept-nav guard: the guard test keeps the alias identifier
    // verbatim (`null == g.window ? void 0 : _self`), so the alias-keep contract holds, while
    // the ctor-swap / hop-drop reconstructions below would erase the probe's short-circuit and
    ...
    // the unroll drops the outer member levels, whose text rides on verbatim in the tail slice - so
    // track the node the surviving plan describes. the leaf's own end is what the tail starts after;
    // reading it off the OUTER `target` swallows every member above a COMPUTED leaf
    let collapseNode = target;
    while (collapse?.kind === 'member') {
      collapseNode = collapse.innerNode;
    ...
    // a kept root re-emits its own source VERBATIM: a kept root is an alias root, so `rootBinding` is
    // the root's own text and a splice around it would rewrite nothing. the raw root inside still gets
    // polyfilled - by the live natural-visitor rewrite that composes into this kept slice by needle
    const keepText = keepRoot && `(${ src.slice(keepRoot.start - baseStart, keepRoot.end - baseStart) })`;
    ...
    if (collapse.keyPrefixSE?.length) {
      ...
      const afterLeaf = src.slice((collapse.computed ? collapseNode.end : prop.end) - baseStart);
  ...
  function collapseProxyHopRoot(metaPath) {
    ...
    if (target?.type === 'ObjectPattern' && claimedDestructurePatterns.has(target)) {
      ...
      if (!consumedStaticResidualPatterns.has(target)) {
        // an SE-computed-key extraction keeps the WHOLE pattern in place (value renamed to a
        // sentinel), so the source read survives as its residual and no pipeline channel
        // re-renders the init. when that surviving nav short-circuits over an undefinable
        // probe (a live `?.` or a sealed paren layer), deferring leaves only the root-only
        // rewrite - a raw polyfillable hop read off a defined receiver (`_globalThis.window
        // ?.self...`, undefined on the ponyfill's target class). render the kept-nav canon
        // here instead (guard-value probe, ponyfill hop, plain-or-optionalized tail) - the
  ...
  function synthMemoArgSrc(memoNode, ctx) {
    ...
    const ctorSwap = proxyGlobalMemberCtorPureSwap({
      receiver: tail, aliasCtx: ctx, resolvePure,
    });
  ...
  function applySynthSwaps() {
    for (const [, pendingSwap] of pendingSynthSwaps) {
      ...
      // no apply-time shape re-checks: unlike the AST emitter, nothing can change a registered
      // node between `tryRegisterPerBranchSynth` and here (no tree mutation, no re-traversal), so
      // the registration gates (`isViableBranchForKey` and the pattern test there) are the whole
      // contract - a silent `continue` here could only mask a registration bug as a dead import
      ...
      // a LogicalExpression peels to itself, so an operator discriminator here decides nothing -
      // the `&&` / `||` split belongs to the branch resolvers that pick the operand, not to a peel
      ...
      const ctorPure = receiverPure ? null
        : proxyGlobalMemberCtorPure({ receiver: readReceiver, aliasCtx: ctx, resolvePure });
      ...
      // drops the read the source performs on the sealed VALUE - re-emit it as a THROW probe
      // (carrying the nav's key SE - it replaces the whole-receiver rescue re-emit) ahead of
      // the literal; the text render substitutes the raw proxy root itself
      // a fallback-LOGICAL receiver probes through its peeled LEFT branch (the branch the
      // value reads through; the collapse supplants exactly that read)
      const throwProbe = !needMemo ? sealedThrowProbePrefix(readReceiver, path) : null;
      const body = needMemo
        ? `(function (${ memoName }) { return ${ literal }; })(${ synthMemoArgSrc(memoNode, ctx) })`
        : throwProbe ? `(${ throwProbe.src }, ${ literal })`
      ...
      const leftSe = needMemo ? [] : (leftSePlan ?? [])
        // SE nodes the sealed throw probe already carries must not re-emit ahead of it
        .filter(node => !throwProbe?.keySeExprs.includes(node))
        .map(node => nodeSrc(node));
  ...
  function applyDestructuringTransforms() {
    // `composedRangeSrc` is DESTRUCTIVE - it drains the queued rewrites inside the range - so a second
    // compose of the same receiver returns raw source, silently losing the natural visitor's rewrites.
    // ONE flush-scoped memo across all three drains below: they used to keep a private cache each, so a
    // receiver reachable from two of them was composed twice. `onFirst` carries whatever range action
    // the drain that got there first owes (the memo's `_ref` overwrite, the copy's re-add)
    const composedReceiverText = new Map();
    function composeReceiverOnce(receiverNode, onFirst = null) {
      const key = `${ receiverNode.start }:${ receiverNode.end }`;
      if (composedReceiverText.has(key)) return composedReceiverText.get(key);
      renderNestedDestructureStatements(receiverNode.start, receiverNode.end);
      const text = composedRangeSrc(receiverNode);
      composedReceiverText.set(key, text);
      onFirst?.(text);
      return text;
    }
    ...
    for (const { start, end, prefix, binding, receiverNode, lhsNode, defaultSrc, testRef, instanceWrap } of pendingReceiverExtracts) {
      const receiverSrc = composeReceiverOnce(receiverNode);
      const lhsSrc = lhsNode ? `${ composedRangeSrc(lhsNode) } = ` : '';
      const call = instanceLeafSrc(instanceWrap, `${ binding }(${ receiverSrc })`);
    ...
    for (const memo of pendingReceiverMemos) {
      const text = composeReceiverOnce(memo.receiverNode,
    ...
    for (const { receiverNode, emit } of pendingReceiverCopies) {
      ...
      emit(composeReceiverOnce(receiverNode,
        text => transforms.add(receiverNode.start, receiverNode.end, text)));
    }
    pendingReceiverCopies.length = 0;
    // both registration phases are complete - flush the order-buffered preceding extracts
    // (stable sort keeps registration order within one prop's entries)
    pendingPrecedingExtracts.sort((a, b) => a.insertPos - b.insertPos || a.order - b.order);
    for (const e of pendingPrecedingExtracts) transforms.insert(e.insertPos, e.text);
    pendingPrecedingExtracts.length = 0;
    ...
    for (const { node, before, after } of pendingBodylessBlockWraps.values()) {
      // same source-order rule as the flushed preceding-extract buffer: entries without an
      // order (null) keep registration order ahead of ordered ones
      const beforeText = before
        .map((e, seq) => ({ ...e, seq }))
        .sort((a, b) => (a.order ?? -1) - (b.order ?? -1) || a.seq - b.seq)
        .map(e => e.text).join('');
