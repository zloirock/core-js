...
- `ast/print.js`, `ast/entry.js` - the AST engine (the staged migration off the text layer): the esrap printer adapter (loc synthesis, paren normalization to the minimal structural set, the corpus-measured esrap gap overrides) and the body-surgery applies - `planEntries`' plan for entry-global, the swept-import splice plus module injection for usage-global (single pass only until the pre/post snapshot lands). The `engine` option selects the engine; `'ast'` accepts the landed methods - entry-global and usage-global today - and rejects the rest, phased usage configurations included. The detection-convenience tree mutations (`neutralizeUnwalkedParamPatterns`) record undo thunks the print replays first. Nothing under `ast/` imports the text-transform layer - lint-enforced, so that layer can one day be deleted out from under it
