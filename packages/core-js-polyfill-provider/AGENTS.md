...
## Statics and globals: realistic spellings only

Statics, globals and global proxies are not reached the way instance methods are. An instance method's receiver arrives through arbitrary data flow, and inferring its type over that flow is the product; a static is spelled where it is used. Almost every use reads it directly (`Array.from`, `new Promise`); fewer go through a shortcut an assignment or a destructuring took (`const P = Promise`, `const { from } = Array`); fewer still through one level of a global proxy and/or a condition, whether it selects the value or checks its presence (`globalThis.Map`, `(c ? Map : Set).groupBy`, `typeof Promise != 'undefined' && Promise.resolve()`, `Array.from ? Array.from(x) : fallback`); every other route is next to nonexistent in real code. Writes to them, which the mutation census tracks, follow the same distribution.

Detection, the census and the transforms cover that distribution in its order and stop there. A rarer or unrealistic route - a static reached through a written container slot, a call result, a factory, a spread, a deeper proxy chain, a patch routed through a function parameter or an aliased `.call` - is inferred or rewritten only where it comes (almost) free out of canon the realistic spellings already use: never through logic of its own, and never at a transform-time cost, since time on real code outranks precision on shapes nobody writes. Past that point a route is not supported: handling that already exists for it and costs anything is removed rather than kept, and the route takes whatever answer the remaining canon gives it - an accepted limitation, not a defect. A probe that finds such a route has not found a bug: estimate its runtime cost and implementation complexity, and bring both to a maintainer before implementing anything.
...
Preserve receiver evaluation, computed-key effects and binding writes in source order. A sequence prefix originally preceding the pattern must precede extracted writes too; use `dropDeadSequenceElements` to discard only unobservable work. Retained captures serve hosts where splitting would reorder getters, keys or assignments. Exports keep their original public names separate from private captures. An init that runs code - a call, a getter, an effect inside the read itself - evaluates before the pattern binds anything: `residualInitRunsEffects` puts the extractions behind a surviving residual, which then keeps its own prefix, and both legs ask it of the pristine init, before a rewrite respells it. Validate ordering with runtime effect logs, not just import sets.
...
Mutation records belong to declarations and their scope chains, not names. Follow aliases and returned containers through the same canon for reads, writes and escapes (`calleeYieldedContainer`). Unknown container keys keep the whole-family fallback, except proven constructor selections for named static reads; bare realm/proxy keys remain tracked positions, while an unfoldable key off a constructor itself (`Map[k]`) may read any of its statics and holds the whole family, as its alias, proxy-member (`globalThis.Map[k]`) and destructure spellings do; a presence test by such a key (`k in Map`) holds it too, since without every static its answer is wrong, but hands no container slot out. The census and the claims fold keys through separate canons, and a read counts as served by its key only where both name it: a key one canon folds and the other cannot counts as unfoldable once it may name a static. Reassignment, slot writes and opaque handouts invalidate narrow proofs. Local object methods are owned by receiver binding plus key: escaped owners or unaccounted reads prevent a closed caller census; absence of constructor rebinding does not close it. `argsUnknown` is acceptable when recording known arguments, never when proving a slot empty.

A pattern slot read as proven (`certainPairedSlotValue`) holds its one paired value only where no default may fire in its place and no spread ahead of it may shift it. Follow an initializer only after it has run (`varInitDominatesUsage`); a hoisted `var` can still be undefined at an earlier read. This applies to aliases, container slots, folded keys, inline callees and defaults. Global import-only analysis may keep following where pure substitution must refuse.
...
- **Value-flow limits can retain pure substitution over a user write.** Library callbacks and values beyond the composite-depth budget are outside the precise model; the coarse global census remains conservative. A function parameter holds only its own default in both censuses: what a call passes, `arguments[i]` and a forwarded or `super` argument included, is not followed. Do not reopen the rejected workaround of deopting a key across every constructor: an unrelated `opts.assign` write must not disable `Object.assign` for the file.
...
- **A read deferred into a function body or default is taken as initialized.** A call made before the binding it reads leaves its TDZ loses the native `ReferenceError` on every spelling - member, destructure, receiver default (`initializerMayRunBeforeUsage`).
...
- **Top-level `this` is the realm.** Detection and type resolution read a module-level `this` as the global proxy in every source type (`isTopLevelThisContext`); in ESM a chain off it then yields the polyfilled member where native throws `TypeError`. Function, class and TypeScript namespace or enum bodies rebind it.
