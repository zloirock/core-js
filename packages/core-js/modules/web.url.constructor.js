...
var utf8PercentEncode = function (char, set) {
  var code = codeAt(char, 0);
  // percent-encoding leaves ' alone, but it belongs to the special-query percent-encode set
  return code >= 0x20 && code < 0x7F && !hasOwn(set, char) ? char : char === "'" && hasOwn(set, char) ? '%27' : percentEncode(char);
};
  ...
  parse: function (input, stateOverride, base) {
    ...
    while (pointer <= codePoints.length) {
      ...
      switch (state) {
        ...
        case SCHEME:
          if (char && exec(ALPHANUMERIC_PLUS_MINUS_DOT, char)) {
            buffer += toLowerCase(char);
          } else if (char === ':') {
            if (stateOverride && (
        ...
        case SPECIAL_AUTHORITY_SLASHES:
          state = SPECIAL_AUTHORITY_IGNORE_SLASHES;
          if (char !== '/' || codePoints[pointer + 1] !== '/') continue;
          pointer++;
          break;
        ...
        case PATH:
          if (
            ...
            buffer += utf8PercentEncode(char, pathPercentEncodeSet);
          } break;
        ...
        case CANNOT_BE_A_BASE_URL_PATH:
          if (char === '?') {
            ...
            if (char === ' ') {
              ...
              url.path[0] += utf8PercentEncode(char, C0ControlPercentEncodeSet);
            }
          } break;
        ...
        case QUERY:
          if (!stateOverride && char === '#') {
            ...
            url.query += utf8PercentEncode(char, url.isSpecial() ? specialQueryPercentEncodeSet : queryPercentEncodeSet);
          } break;

        case FRAGMENT:
          if (char !== EOF) url.fragment += utf8PercentEncode(char, fragmentPercentEncodeSet);
          break;
      }
