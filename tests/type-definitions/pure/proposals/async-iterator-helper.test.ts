...
drop(aiton, 3);
const r2: CoreJSPromiseOrPromiseLike<boolean> = every(aiton, (v: number, i: number) => v > 0);
filter(aiton, (v: number, i: number) => v > 0);
const r4: CoreJSPromiseOrPromiseLike<number> = find(aiton, (v: number, i: number) => v > 0);
flatMap(aiton, (v: number, i: number) => `${v}`);
const r6: CoreJSPromiseOrPromiseLike<void> = forEach(aiton, (v: number, i: number) => { });
map(aiton, (v: number, i: number) => v * 2);
const r8: CoreJSPromiseOrPromiseLike<number> = reduce(aiton, (acc: number, v: number, i: number) => acc + v, 0);
const r9: CoreJSPromiseOrPromiseLike<boolean> = some(aiton, (v: number, i: number) => v > 0);
take(aiton, 10);
const r11: CoreJSPromiseOrPromiseLike<number[]> = toArray(aiton);
...
const s0: CoreJSPromiseOrPromiseLike<number[]> = toArray(aiton);
const f0: CoreJSPromiseOrPromiseLike<string> = find(aitos, (v: string, i: number) => v.length === 1);
