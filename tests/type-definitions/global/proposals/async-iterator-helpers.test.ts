...
declare const ains: AsyncIteratorObject<number | string>;
...
AsyncIterator.from(is);
AsyncIterator.from(itn);
...
const r3g: AsyncIterator<string> = ains.filter((v): v is string => typeof v === 'string');
const r4: Promise<number | undefined> = ain.find((v: number, i: number) => v > 0);
const r4g: Promise<string | undefined> = ains.find((v): v is string => typeof v === 'string');
