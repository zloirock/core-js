...
- These suites are blind to one node class: their shapes are parsed by BABEL, which drops source parens, so nothing here reaches a `ParenthesizedExpression` - the node the estree adapter's parser keeps. A row spelling parens passes without testing them and a domain entry for the position is never observed, so that class is checked by running the two emitters against each other on the shape, not from here
