...
export const POSITIONS = [
  ...
  ['sequence-alias', 'const held = (log(), @); held.read();', 'local'],
  ['sequence-assignment-alias', 'let held; held = (log(), @); held.read();', 'local'],
  ['captured-pattern-data', 'let held; const { rows } = held = (log(), @); held.read();', 'local'],
  ['pattern-data-export', 'export const { rows: { at } } = @;', 'local'],
  ['nested-pattern-data-export', 'export const { w: { rows: { at } } } = { w: @ };', 'local'],
  ['nested-pattern-method-export', 'export const { w: { read } } = { w: @ };', 'escapes'],
  ['array-pattern-method-export', 'export const [{ read }] = [@];', 'escapes'],
  ['nested-pattern-holder-export', 'export const { w } = { w: @ };', 'escapes'],
  ['nested-pattern-computed-export', 'export const { [key]: { read } } = { w: @ };', 'escapes'],
  ['pattern-method-export', 'export const { read } = @;', 'escapes'],
  ['captured-pattern-method', 'let held; const { read } = held = (log(), @); sink(read);', 'escapes'],
  ['captured-pattern-alias-escape', 'let held; const { rows } = held = (log(), @); sink(held);', 'escapes'],
  ['sequence-assignment-result-escape', 'let held; sink(held = (log(), @));', 'escapes'],
