...
- `warning-policy.mjs` - what a bundler warning does, shared with `tests/e2e-libs`: an unresolved injection turning into an external import is the same failure of the same plugins there, so editing this edits both suites
- `matrix.mjs` - the methods, the phases each one supports, and the plugin options that select them, also shared with `tests/e2e-libs`. Neither it nor `warning-policy.mjs` pulls anything in, which is what lets that suite import them without installing this directory; keep it that way
