...
const arr = [3, [1, 2]];
let out;
if (1) var viaBodylessIf = _atMaybeArray(_flatMaybeArray(arr).call(arr));
do var viaDoWhile = _atMaybeArray(_flatMaybeArray(arr).call(arr)); while (0);
for (var viaForInit = _atMaybeArray(_flatMaybeArray(arr).call(arr)); !out;) out = viaForInit;
let viaWrapAssign;
viaWrapAssign = _atMaybeArray(_flatMaybeArray(arr).call(arr));
const viaDeclarator = _atMaybeArray(_flatMaybeArray(arr).call(arr));
let viaStatement;
viaStatement = _atMaybeArray(_flatMaybeArray(arr).call(arr));
