...
export const plainRead = globalThis.self?.window;
export const typeofRead = typeof globalThis.self?.window;
export const inAnArray = [globalThis.self?.window].length;

// NEGATIVE: a claim reading THROUGH the run consumes it, so the probe folds onto the ponyfill
export const readThrough = globalThis.self?.window.Array.from([1]);
