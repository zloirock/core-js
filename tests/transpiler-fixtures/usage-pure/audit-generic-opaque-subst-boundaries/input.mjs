...
function mix<T, U = number[]>(t: T | null, u: U): U { return u; }
mix(opaque, [1]).at(0);
function partial<T = number[], U = T>(): U { return [] as any; }
partial<Opaque>().includes(1);
function defaultsOnly<U = string[]>(): U { return [] as any; }
defaultsOnly().at(0);
function twice<T = string>(x: T | null): T { return x as any; }
twice(opaque).at(0);
twice('abc').includes('b');
function cyc<T = T[]>(x: T | null): T { return x as any; }
cyc(opaque).at(0);
declare const tup: [string, number];
function head<T extends unknown[]>(t: T): T[0] { return t[0]; }
head(tup).includes('q');
