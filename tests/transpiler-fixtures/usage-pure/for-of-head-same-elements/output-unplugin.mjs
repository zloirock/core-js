...
for (const { is: viaIdentifiers } of [{ is: _Object$is }, { is: _Object$is }]) viaIdentifiers;
for (const { w: { keys: viaHop } } of [{ w: { keys: _Object$keys } }, { w: { keys: _Object$keys } }]) viaHop;
for (const [{ values: viaWrap }] of [[{ values: _Object$values }], [{ values: _Object$values }]]) viaWrap;

for (const { w: [{ entries: viaHopWrap }] } of [
	{ w: [{ entries: _Object$entries }] },
	{ w: [{ entries: _Object$entries }] }
]) viaHopWrap;

for (const { w: { x: { hasOwn: viaTwoHops } } } of [
	{ w: { x: { hasOwn: _Object$hasOwn } } },
	{ w: { x: { hasOwn: _Object$hasOwn } } }
]) viaTwoHops;

// ... read through the transparent wrappers a source may spell (a paren node one parser keeps)
for (const { w: { isFrozen: viaParens } } of [
	{ w: { isFrozen: _Object$isFrozen } },
	{ w: { isFrozen: _Object$isFrozen } }
]) viaParens;
...
for (const { w: { entries: viaDualHop } } of [{ w: { entries: _Object$entries } }]) viaDualHop;

for (const { w: { keys: viaDualMulti } } of [{ w: { keys: _Object$keys } }, { w: { keys: _Object$keys } }]) viaDualMulti;
for (const { w: [{ values: viaDualWrap }] } of [{ w: [{ values: _Object$values }] }]) viaDualWrap;
for (const { w: { entries: viaDualDefault = null } } of [{ w: { entries: _Object$entries } }]) viaDualDefault;
for (const [{ keys: viaDualArrayHead }] of [[{ keys: _Object$keys }], [{ keys: _Object$keys }]]) viaDualArrayHead;
...
// carries no claim
for (const _ref of [{ w: Object, at: 1 }]) {
	let viaDualBesideData = _Object$entries;
	let { at: viaDataBeside } = _ref;
...
for (const viaWritten of [{ w: Object }]) {
	const viaWrittenKeys = _Object$keys;

	viaWrittenKeys;
}

for (const { w: { is: viaPrimitiveSlots }, z } of [
	{ w: { is: _Object$is }, z: 's' },
	{ w: { is: _Object$is }, z: 2 }
]) [viaPrimitiveSlots, z];
...
for (const _ref4 of [{ w: [Object] }, { w: [userObj] }]) {
	let { w: [{ freeze: viaOtherValue }] } = _ref4;

	viaOtherValue;
}

for (const _ref6 of [{ w: Object }, { v: Object }]) {
	let { w: _ref5 } = _ref6,
		viaOtherKey = _ref5 === Object ? _Object$seal : _ref5.seal;

	viaOtherKey;
}

for (const { w: { assign: viaGetter } } of [
	{ w: Object },
	{
		get w() {
			return Object;
...
for (const { w: { groupBy: viaExtraSlot } } of [
	{ w: { groupBy: _Object$groupBy }, z: 1 },
	{ w: { groupBy: _Object$groupBy }, z: 2 }
]) viaExtraSlot;

for (const _ref8 of [{ w: Object }, { w: Object, ...more }]) {
	let { w: _ref7 } = _ref8,
		viaSpread = _ref7 === Object ? _Object$fromEntries : _ref7.fromEntries;
...
for (const viaLater of [{ w: Object }]) {
	const z = 1;
