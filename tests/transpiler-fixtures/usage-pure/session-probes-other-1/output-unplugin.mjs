...
{
	class C1 {
		constructor([{ from: f }] = [pick ? { from: _Array$from } : userObj]) {
			this.f = f;
		}
	}
...
{
	const [, { from: f }] = [eff(), { from: _Array$from }];
}

{
	const [, { from: f }] = [eff(), pick && { from: _Array$from }];
}

{
	const [, { from: f }] = [eff(), pick ? { from: _Array$from } : userObj];
}
...
{
	const [{ Array: { from: f } } = {}] = [pick ? { Array: { from: _Array$from } } : _Set];
}
...
{
	const [{ a: [{ hasOwn }] }] = [{ a: [{ hasOwn: _Object$hasOwn }] }];
}
...
{
	const [_ref9] = [[eff()], ...rest];
	const at = _atMaybeArray(_ref9);
}

{
	const a = _atMaybeArray(arr);
	const [{}, { from: f }] = [arr, pick ? { from: _Array$from } : userObj];
