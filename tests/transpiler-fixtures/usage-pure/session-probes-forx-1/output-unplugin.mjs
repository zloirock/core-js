...
{
	for (const { is } of [{ is: _Object$is }, { is: _Object$is }]) is;
}

{
	for (const { is } of [{ is: _Object$is }]) is;
}
...
{
	for (const [[{ entries }]] of [[[{ entries: _Object$entries }]]]) entries;
}
...
{
	for (const [{ hasOwn }] of [[{ hasOwn: _Object$hasOwn }], [{ hasOwn: _Object$hasOwn }]]) hasOwn;
}
...
{
	for (const [{ keys }] of [[{ keys: _Object$keys }], [{ keys: _Object$keys }]]) keys;
}

{
	for (const [{ values = null }] of [[{ values: _Object$values }]]) values;
}
...
{
	for (const [{ values }] of [[{ values: _Object$values }]]) values;
}
...
{
	for (const _r of [{ w: Object }, { w: userObj }]) {
		let { w: _ref11 } = _r,
...
{
	for (const _r of [{ w: Object }]) for (const _s of [_r]) {
		let keys = _Object$keys;
...
{
	for (const _ref14 of [_globalThis, { Array }]) {
		let { Array: _ref13 } = _ref14,
