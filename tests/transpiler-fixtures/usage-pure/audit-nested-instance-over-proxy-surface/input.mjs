...
const { Array: { prototype: { flat: fromGlobal } } } = globalThis;
const { prototype: { at: fromCtor } } = Array;
const { Array: { prototype: { includes: fromNav } } } = globalThis.globalThis;
// the FOR-INIT row drops its slot like any other: what the drop needs is a DECLARATOR slot, not a
...
let inLoop;
for (const { Array: { prototype: { findLast: loopBound } } } = globalThis; !inLoop;) inLoop = typeof loopBound;
// a DEFAULTED leaf keeps its guard: the dispatch answers `it.method` verbatim off a foreign surface
const { Array: { prototype: { toSorted: defaulted = null } } } = globalThis;
// the SYMBOL leaf under the same surface rides the same route: its receiver is that surface, not the
// object above it
const { Array: { prototype: { [Symbol.iterator]: symLeaf } } } = globalThis;
// NEGATIVE: a chain of pristine proxy names peels away entirely and reads the ROOT itself, where the
// native slot stands - `keys` off the global object is not a claim on a polyfilled surface
const { self: { keys: offRoot } } = globalThis;
...
const { Array: { keys: onCtor } } = globalThis;
let onCtorAssign;
({ Array: { keys: onCtorAssign } } = globalThis);
