...
let e = 0;
export function viaParam(globalThis) {
  return delete globalThis.self.customProp;
}
export function viaHopName(self) {
  return delete self.window.customProp;
}
export function viaLexical() {
  const globalThis = {
    self: {}
  };
  return delete globalThis.self.customProp;
...
export function viaNested(globalThis) {
  return () => delete globalThis.self.customProp;
}
export function viaRead(globalThis) {
  return globalThis.self.customProp;
}

// NEGATIVE: the same shapes off the REAL realm still fold - the shadow is what stops the collapse,
// not the spelling
export const realDelete = delete _globalThis.customProp;
export { e };