...
let q8, onx, out8;
for (const _ref4 = (({ ononline: onx } = (q8 = _globalThis, _globalThis)), Object), fk = _Object$keys; !out8;) out8 = fk;
...
let customY2, outFC;
for (const g2 = _Map$groupBy, _unused = (({ onlanguagechange: customY2 } = _globalThis), _globalThis); !outFC;) outFC = g2;
...
let customFR, outFR;
for (const { customFR: fr } = (({ self: { onoffline: customFR } } = _globalThis), _Promise); !outFR;) outFR = fr;
...
let itF, outSF;
for (const _ref3 = (({ WeakSet: { [_Symbol$iterator]: itF } } = _globalThis), Object), gopn = _Object$getOwnPropertyNames; !outSF;) outSF = gopn;
...
const sA = sideA();
const { onresize: onrs } = (eff(), _globalThis);
const sB = sideB();
// export host keeps the re-export wrap around the inline-replayed residual
export const { customEX } = (eff(), _WeakMap);
// consecutive proxy hops peel to the ctor anchor with the prefix replayed once
const { customML } = (eff(), _globalThis.RegExp);
