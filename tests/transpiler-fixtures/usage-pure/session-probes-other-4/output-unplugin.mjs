...
{
	const k = 'z';
	const wrapper = { w: _globalThis, z: [1, 2] };
...
{
	const q = 2, { [(k(), 'at')]: s } = eff();

	s(q);
}

{
	const q = 2, { [(k(), 'at')]: s, z } = eff();

	s(z, q);
}

{
	const q = eff0(), { [(k(), 'at')]: s } = eff();

	s(q);
}

{
	const q = eff0(), { [(k(), 'at')]: s, z } = eff();
...

{
	...
	const r5 = o == null
		? void 0
		: _at(_ref7 = o[(eff(), 'data')]).call(_ref7, 0);
}
...
{
	const { '0': { from: f } } = [pick ? { from: _Array$from } : userObj];
}
...
{
	const { 1: { from: f } } = [0, pick ? { from: _Array$from } : userObj];
}

{
	const { Array: _ref9 } = _globalThis,
		_ref10 = _ref9,
		f6 = null == _ref10 ? _ref10[""] : (eff('k'), _Array$from);
}

{
	const { Array: _ref11 } = _globalThis,
		_ref12 = _ref11,
		f = null == _ref12 ? _ref12[""] : (eff('k2'), _Array$from);
}

{
	const { Array: { [(eff('k2'), 'from')]: f }, ...r } = _globalThis;
}

{
	const { Array: { from } } = id(_globalThis);
}

{
	const { Array: { from }, ...rest } = _globalThis;
...
{
	const { Array: { from: f }, ...r } = _globalThis;
}
...
{
	const { Array: { from: f2 }, ...r2 } = _globalThis;
}