...
export const probeLeafRead = globalThis.self.window?.WeakRef;
export const probeLeafClaim = globalThis.self.window?.Symbol?.iterator;
export const probeLeafTail = globalThis.self.window?.Array?.prototype;
// NEGATIVE: no `?.` at all - the whole navigation collapses, probe included (the multihop canon)
export const plainDeepNav = globalThis.self.window.customKey;
...
export const firstHopProbe = globalThis.window.self.window?.BigInt;
