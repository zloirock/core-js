...
labeled: { var viaLabel = _Symbol$iterator; }
export const viaLabeledBlock = _getIteratorMethod([]);

function throughFinally() {
  try { throughFinally.touched = true; } finally { var fin = _Symbol$asyncIterator; }
  return [][fin];
}
export const viaFinally = throughFinally();

for (var forInit = _Symbol$iterator; Math.random() > 2;) { break; }
...
function branchDecl(flag) {
  if (flag) { var iterator = _Symbol$iterator; }
...
function loopDecl() {
  while (Math.random() > 2) { var iterator = _Symbol$iterator; }
  return _getIteratorMethod([]);
}
export const viaLoopDecl = loopDecl();

var guardedAssign;
if (Math.random() > 2) { guardedAssign = _Symbol$iterator; }
