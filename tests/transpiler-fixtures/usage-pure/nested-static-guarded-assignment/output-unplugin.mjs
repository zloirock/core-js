...
try {
	...
	({
		...
		[(
			_pushMaybeArray(events).call(events, _atMaybeString(_ref = 'x').call(_ref, 0), typeof from),
			'Object'
		)]: { keys }
	} = (
...
try {
	...
	({
		...
		Object: {
			keys: box[(
				_pushMaybeArray(events).call(events, method === pureFrom, _atMaybeString(_ref2 = 'y').call(_ref2, 0)),
				'value'
			)]
		}
	} = (
		_pushMaybeArray(events).call(events, 'partial-source'),
		(null == _globalThis.window ? void 0 : _self).Array,
		{
			Array: { from: _Array$from },
			Object: { keys: _self.Object.keys }
		}
	));
} catch(error) {
