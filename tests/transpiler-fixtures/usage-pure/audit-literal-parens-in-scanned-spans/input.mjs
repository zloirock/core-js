...
export const keyParen = globalThis.window?.Object.entries({ ')': 1 }).flat();
...
export const nestedQuote = globalThis.self.JSON.stringify({ a: '")' }).includes('a');
...
let seCount = 0;
export const { hypot = ')' } = (seCount++, Math);
let seCount2 = 0;
export const { trunc = '} = x' } = (seCount2++, Math);
