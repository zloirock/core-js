...
const { from: a1 } = cond ? { from: _Array$from } : _Map;
// VariableDeclarator init: ||, mixed viable (Array.from yes, userObj unknown)
const { from: a2 } = userObj || { from: _Array$from };
// VariableDeclarator init: ??, member-call left (not Identifier - left stays raw)
const { from: a3 } = pickConstructor() ?? { from: _Array$from };
// AssignmentExpression: ternary, multi-key destructure
let b1, b2;
({ from: b1, of: b2 } = cond ? { from: _Array$from, of: _Array$of } : _Map);
// AssignmentPattern (function param default): && reversed
function f({ groupBy } = _Promise && { groupBy: _Map$groupBy }) { return groupBy; }
// AssignmentPattern: nested conditional - outer ?? wraps inner ?
function g({ try: t } = (cond1 ? _Promise : null) ?? { try: _Promise$try }) { return t; }
