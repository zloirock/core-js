...

{
	...
	_ref = { w: [1] };
	({ [(eff('k'), 'w')]: _ref2 } = null == _ref ? _ref[""] : _ref);
	(f17 = _atMaybeArray(_ref2), _ref);
}
...

{
	...
	({ [(eff('k'), 'Array')]: _unused } = _globalThis);
	f9 = _Array$from;
}

{
	...
	({ w: { from: f } } = { w: pick ? { from: _Array$from } : userObj });
}

{
	let f;

	[{ from: f }] = [pick ? { from: _Array$from } : userObj];
}

{
	let from, rest;

	({ Array: { from }, ...rest } = _globalThis);
	use(from, rest);
}
...

{
	...
	({ w: { Map: m }, ...rest } = { w: _globalThis, z: 1 });
	use(m, rest);
}
...

{
	try {
		throw 0;
	} catch({ [(eff('k'), 'Array')]: { from: f } = _globalThis }) {
		f;
	}
}
