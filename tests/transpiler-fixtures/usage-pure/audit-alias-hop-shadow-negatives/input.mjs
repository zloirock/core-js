...
export function viaMultiDeclLastShadowed() {
  ...
  { const multiRoot = {}; var multiHeld = multiRoot; }
  { const { allSettled } = multiHeld; return allSettled([]); }
}

// a for-init `var` whose init reads a block-local shadow
const forRoot = Map;
export function viaForInitVarShadow() {
  { const forRoot = {}; for (var forHeld = forRoot; false;) break; }
  { const { groupBy } = forHeld; return groupBy([], v => v); }
}
...
export function viaNestedFnVar() {
  function inner() { var nestedHeld = nestedRoot; return nestedHeld; }
  { const { race } = nestedHeld; return race([]); }
}
...
export function viaVarReassignedInBlock() {
  { var reassignHeld = reassignRoot; reassignHeld = {}; }
  { const { fromEntries } = reassignHeld; return fromEntries([]); }
}
