...
let q;
export const viaSelfHop = _flatMaybeArray((q = _self, _self).Array.prototype);

// a call through the tail keeps its receiver binding and drops the same trailing hop
let w;
export const viaSelfCallHop = _includesMaybeArray((w = _self, _self).Array.prototype).call([1, 2], 2);
...
let e = 0;
let s;
export const viaSeqSelfValue = _atMaybeArray((s = (e++, _self), _self).Array.prototype).call([1], 0);
export { e };

// a deeper all-ponyfilled navigation as the value: the leaf (`globalThis`) is the spelling
let d;
export const viaDeepPonyValue = _mapMaybeArray((d = _globalThis, _self).Array.prototype).call([1], x => x);
...
let u;
export const viaUnresolvedMethod = (u = _self, _self).Array.prototype.lastIndexOf.call([1], 1);