...
export function notFalse(input: string | number[]) {
  if (obj.isStr?.(input) !== false) return _at(input).call(input, 0);
  return null;
}
export function isFalseElse(input: string | number[]) {
  if (obj.isStr?.(input) === false) return null;
  return _at(input).call(input, 1);
}
export function isTrue(input: string | number[]) {
  if (obj.isStr?.(input) === true) return _includesMaybeString(input).call(input, 'a');
  return null;
}