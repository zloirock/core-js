...
// literal is memoized into a single `_ref` that both the `_at...(_ref)` body-extract and the residual
// destructure reference; a pure literal (no inner side effect) may instead be re-emitted safely.
// guards `isReReferenceableReceiver` recursing into the literal's elements, not just the literal node
