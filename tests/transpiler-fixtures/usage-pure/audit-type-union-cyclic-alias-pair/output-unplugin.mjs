...
type Ping = Pong | { kind: "a"; items: string[] };
type Pong = Ping | { kind: "b"; items: string[] };
type Loop = Ring | { kind: "a"; entries: string[] };
type Ring = Loop | { kind: "b"; entries: string[] };
...
export function bothHalvesOfACyclicPair() {
	if (pair.kind === "a") {
		...
		return _atMaybeArray(_ref = pair.items).call(_ref, 0);
	}
...
export function cycleReachedThroughAUnionAlias() {
	if (viaUnion.kind === "a") {
		...
		return _includesMaybeArray(_ref2 = _entries(viaUnion)).call(_ref2, "x");
	}
