...
const [{ Array: { [key()]: extracted } } = {}] = [pick ? { Array: { from: _Array$from } } : _Set];

const [_ref] = [pick ? { Array: { from: _Array$from } } : _Set],
	{ Array: { [key()]: noDefault } } = _ref;

const { Array: { [key()]: noWrapper } } = pick ? { Array: { from: _Array$from } } : _Set;
const { Array: { [key()]: valueDefault } = {} } = pick ? { Array: { from: _Array$from } } : _Set;
const { Array: { [k('from')]: viaIdentity } } = pick ? { Array: { from: _Array$from } } : _Set;

// the control: the literal spelling of the same effectful key
const [{ Array: { [(calls += 1, 'from')]: literalKey } } = {}] = [pick ? { Array: { from: _Array$from } } : _Set];
