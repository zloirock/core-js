...
export interface Box<Set> { v: number }
...
export class Overloaded {
  ...
  m(Symbol: any) { return Symbol; }
}
export abstract class Base { abstract run(Promise: number): void; }
export interface Keys { [ArrayBuffer: string]: number; Reflect: number; [Map.name]: number }
export type Mapped = { [BigInt in 'a']: 1 };
declare namespace NS { type Uint8Array = number }
export type Member = NS.Uint8Array;
export function guard(WeakRef: any): WeakRef is number { return true; }
export declare const annotated: DisposableStack;
export const cast = value as AggregateError;
export const s = new Set([1]);
export class Keyed { [Promise.name] = 1; }
