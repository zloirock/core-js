...
type Twin = 
	{ kind: "a"; entries: string[] } |
	{ kind: "b"; entries: string };

type SharedBranches = Inner | Inner;
type DistinctBranches = Twin | { kind: "c"; entries: string[] };
...
export function branchesShareOneUnionDecl() {
	if (shared.kind === "a") {
		...
		return _atMaybeArray(_ref = shared.items).call(_ref, 0);
	}
...
export function branchesUseDistinctDecls() {
	if (distinct.kind === "a") {
		...
		return _includesMaybeArray(_ref2 = _entries(distinct)).call(_ref2, "x");
	}
