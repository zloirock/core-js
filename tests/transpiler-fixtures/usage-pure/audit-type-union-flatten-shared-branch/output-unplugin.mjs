...
export function branchesShareOneUnionDecl() {
  if (shared.kind === "a") { var _ref; return _atMaybeArray(_ref = shared.items).call(_ref, 0); }
  return "";
}

// the control: one declaration per branch, so nothing is re-reached either way
export function branchesUseDistinctDecls() {
  if (distinct.kind === "a") { var _ref2; return _includesMaybeArray(_ref2 = _entries(distinct)).call(_ref2, "x"); }
  return false;
}