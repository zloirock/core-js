...
const { w: { WeakSet: keptByGetterEffect } } = {
	get w() {
		...
		return { WeakSet: _WeakSet };
	}
};

function keptBySpread(extra) {
	const { w: { Array: _ref } } = { w: _globalThis, ...extra },
...
function seqPrefixKeepsLiteral(bump) {
	...
	const { w: { [_Symbol$iterator]: _unused } } = { w: (bump(), _globalThis) };
...
function keptByUnnameableKey(key) {
	...
	const { w: _ref3 } = { w: _globalThis, [key]: other },
		keptDecl = _ref3 === _globalThis ? _Map : _ref3.Map;

	let keptAssign;

	(
		{ w: _ref4 } = { w: _globalThis, [key]: other },
...
function paramHop({ w: { Map: viaParamCtor } } = { w: { Map: _Map } }) {
	return viaParamCtor;
}

function paramHopStatic(
	{ w: { Array: { from: viaParamStatic } } } = { w: { Array: { from: _Array$from } } }
) {
	return viaParamStatic;
}

function paramHopInstance(
	{ w: { at: viaParamInstance } } = { w: { at: _atMaybeArray([1, 2]) } }
) {
	return viaParamInstance;
}

function paramHopWrapped([{ w: { Map: viaParamWrapped } }] = [{ w: { Map: _Map } }]) {
...
function forOfHop() {
	...
	for (const { w: { Map: viaHeadCtor } } of [{ w: { Map: _Map } }]) _pushMaybeArray(out).call(out, viaHeadCtor);

	return out;
}

const viaIifeCtor = (({ w: { Map: m } }) => m)({ w: { Map: _Map } });

// ... and an ARRAY wrapper on the way is one more hop of the same descent
function paramWrappedInstance(
	[{ w: { at: viaWrappedInstance } }] = [{ w: { at: _atMaybeArray([1, 2]) } }]
) {
...
const viaIifeWrappedStatic = (([{ w: { Array: { from: m } } }]) => m)([{ w: { Array: { from: _Array$from } } }]);
...
function boundKeyParam(
	{ [hopKey]: { Map: viaBoundKeyParam } } = { w: { Map: _Map } }
) {
	return viaBoundKeyParam;
}

const viaBoundKeyIife = (({ [hopKey]: { at: m } }) => m)({ w: { at: _atMaybeArray([1, 2]) } });
const viaSeqArg = (({ at: m }) => m)((mark(), { at: _atMaybeArray([1, 2]) }));
const viaIifeInstance = (({ w: { at: m } }) => m)({ w: { at: _atMaybeArray([1, 2]) } });
