...
function observe(read) {
  try { log.push(typeof read()); } catch { log.push('tdz'); }
}
let result;
for (const { Object: { keys }, Array: { prototype: { at } } } = (observe(() => keys), globalThis); !result;) result = [keys, at];
let single;
for (const { Array: { of } } = (observe(() => of), globalThis); !single;) single = of(1);
