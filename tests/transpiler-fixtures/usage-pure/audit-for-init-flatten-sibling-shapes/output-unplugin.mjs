...
for (const from = _Array$from, at = _at(arr), { at: _unused2, ...rest } = arr; cond(); next()) use(from, at, rest);
for (const of2 = _Array$of, flat = _flatMaybeArray(arr), { plain } = arr; cond(); next()) use(of2, flat, plain);
for (const { isArray } = _globalThis.Array, inc = _includes(arr); cond(); next()) use(isArray, inc);
for (const fromAsync = _Array$fromAsync, { indexOf, lastIndexOf } = arr; cond(); next()) use(fromAsync, indexOf, lastIndexOf);
...
for (const entries = _Object$entries, findLast = (_ref2 = _findLastMaybeArray(arr)) === void 0 ? dflt : _ref2; cond(); next()) use(entries, findLast);
// an SE-bearing receiver keeps its evaluation point through the sink declarator
for (const keys = _Object$keys, _ref = getArr(), findIndex = _findIndexMaybeArray(_ref), { findIndex: _unused, ...r2 } = _ref; cond(); next()) use(keys, findIndex, r2);