...
- Multi-line fixtures use a different method per line, and a method that should demonstrate inference needs receivers from several families. In `usage-global` the per-line distinctness is load-bearing, not style: injection is observable only through the import set - the source is not rewritten - so two sites sharing a method produce one import and mask each other's regression. The same masking runs one level wider than the method, because what `usage-global` observes is the MODULE: a read of a proxy global pulls its whole FAMILY, so a negative row sitting beside a positive on that same global distinguishes nothing however differently the two are spelled. And the runner's exit code cannot decide any of this - `usage-global` prints the source verbatim, so ANY edit to `input.mjs` reddens the run whether or not the injection moved. A seed-prove here compares the IMPORT SET of the broken and the intact source; a red run proves the text changed, not that the row covers its claim
...
- A directory name says the SHAPE under test or the negative it pins, never the bug that prompted it: a bug name goes stale the day the bug is fixed, and the fixture outlives it. Rename one you find in the same edit that fixes the behaviour
- Split by CLAIM, not by line count, and move the expected outputs with the input: every separated file keeps the leg-parity check of its own, or the split hides a divergence where the merged file showed one
