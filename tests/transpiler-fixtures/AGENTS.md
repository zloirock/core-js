...
- A sidecar is a proof obligation: show what the divergence actually IS before accepting it. The unplugin leg is compared STRUCTURALLY against the babel baseline (both are AST renderers - formatting is normalized away by `tests/unplugin/structural.mjs`), so an `output-unplugin.mjs` sidecar is never cosmetic: it records an environmental divergence - targets resolution babel@8 does differently, the `require` import dialect on SFC virtuals, an abstain that keeps the user's bytes where babel reprints. Each one is debt to justify, not formatting to skim past
