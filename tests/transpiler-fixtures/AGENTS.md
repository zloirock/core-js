...
- A sidecar is a proof obligation: show what the divergence actually IS before accepting it - and most of them under `usage-pure` are the printer, not a decision. The AST leg reprints through `@babel/generator`, which breaks an object literal onto one property per line and a block body onto its own lines; the text leg splices into the source and keeps what the input spelled. Any fixture whose input holds an object literal or a braced body differs that way and no emitter change removes it, so read the two files against each other first: every hunk line-breaking means the legs agree, and those sidecars are not debt to re-audit. Under `usage-global` a sidecar is never cosmetic - only the import set is compared there, so it means the emitters inject differently
