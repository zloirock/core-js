...
type Tagged = { kind: 1; a: number[]; } | { kind: 2; a: string; };
type Named = { tag: "x"; items: number[]; } | { tag: "y"; items: string; };
...
if (tagged.kind === typedOnly) { tagged.a.at(0); }
if (tagged.kind === aliased) { tagged.a.includes("x"); }
if (tagged.kind === Kind.First) { tagged.a.find(x => x); }
if (named.tag === `y`) { named.items.padStart(2); }
