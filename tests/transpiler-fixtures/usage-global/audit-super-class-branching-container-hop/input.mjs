...
class ViaSlot extends inSlot.Base { static m() { return super.any([]); } }

const overContainer = c ? { Base: Object } : { Base: Array };
class ViaContainer extends overContainer.Base { static m() { return super.fromAsync([]); } }

const inElement = [c ? Map : Object, 0];
class ViaElement extends inElement[0] { static m() { return super.groupBy([], x => x); } }

const nested = { inner: { Base: (c && Object) || String } };
class ViaNested extends nested.inner.Base { static m() { return super.raw`x`; } }

// NEGATIVE: neither arm carries the static, so the key brings in no module of its own
const noStatic = { Base: c ? Object : Boolean };
class ViaNoStatic extends noStatic.Base { static m() { return super.notAStatic; } }
