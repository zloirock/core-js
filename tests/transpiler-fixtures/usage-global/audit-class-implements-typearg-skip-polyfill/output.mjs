...
// uses Map at runtime). babel's `referenced-identifier check` over-reports Map as referenced
// because TS-stripped output erases the whole TSExpressionWithTypeArguments - without a
// pure-erase ancestor check in `TS type-only identifier check`, Identifier visitor would emit
