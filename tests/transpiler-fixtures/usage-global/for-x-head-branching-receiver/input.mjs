...
const empty = null;
for (const { from } of [flag ? Array : Object]) out.push(typeof from);
for (const { from } of [empty || Array]) out.push(typeof from);
for (const { from } of [empty ?? Array]) out.push(typeof from);
// the rewrite lands on whichever arm names the constructor, not on a fixed side
for (const { from } of [flag ? out : Array]) out.push(typeof from);
...
for (const { from } of [flag ? Array : Object, flag ? Array : Object]) out.push(typeof from);
export { out };
