...
const {
  w: {
    toSorted: noInstanceRowsOverOr
  }
} = {
  w: orRealm
};
const [{
  with: noInstanceRowsOverOrWrapped
}] = [orRealm];
const {
  w: {
    Set: viaOrAliasHop
...
const [{
  AggregateError: viaOrAliasWrapped
}] = [orRealm];
...
const {
  w: {
    findLast: instanceRowsOverLater
  }
} = {
  w: later
};
const [{
  Iterator: viaLaterWrapped
}] = [later];
function assignHops() {
  let m;
  let n;
...
function mark() {}
export default [hopCtor, hopStatic, hopThroughGetter, keptByGetterEffect, keptByBranch, keptBySpread({}), keptByKey('Q'), keptByWrite, noInstanceRowsOverOr, noInstanceRowsOverOrWrapped, viaOrAliasHop, viaOrAliasWrapped, instanceRowsOverLater, viaLaterWrapped, assignHops(), paramStringSlot(), iifeStringSlot, forOfWrappedRealm()];