...
const extracted = { cells: ['a'], peek() { return this.cells.at(0); } };
const m1 = extracted.peek;
m1.call({ cells: 'zz' });
const plucked = { tags: 'ab', scan() { return this.tags.includes('a'); } };
const { scan } = plucked;
scan.call({ tags: ['q'] });
const copied = { nums: [1], pick() { return this.nums.at(1); } };
const twin = { ...copied, nums: 'xy' };
twin.pick();
const called = { list: [2], has() { return this.list.includes(2); } };
called.has.call({ list: 'pq' });
class Inst {
  chars = ['c'];
  look() { return this.chars.at(2); }
}
const inst = new Inst();
const m2 = inst.look;
m2.call({ chars: 7 });
class Stat {
  static words = 'wv';
  static ask() { return this.words.includes('w'); }
}
const m3 = Stat.ask;
m3.call({ words: [3] });
class Proto {
  bits = [4];
  poke() { return this.bits.at(4); }
}
...
const frozen = { keys2: ['k'], grab() { return this.keys2.at(6); } };
const alias = Object.freeze(frozen);
const m5 = alias.grab;
m5.call({ keys2: 'kk' });
const inherited = { seq: 'st', find2() { return this.seq.includes('s'); } };
const heir = {};
...
const direct = { marks: 'mn', test() { return this.marks.includes('m'); } };
direct.test();
function probe(k) {
  const local = { data: [5], read() { return this.data.at(5); } };
  local[k];
