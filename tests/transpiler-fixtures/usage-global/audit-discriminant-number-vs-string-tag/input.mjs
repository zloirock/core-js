...
type Tagged = { kind: 1; payload: number[]; } | { kind: "1"; payload: string; };
type Wide = { tag: 2; items: number[]; } | { tag: 2n; items: string; };
declare const tagged: Tagged;
declare const wide: Wide;
if (tagged.kind === 1) { tagged.payload.at(0); }
if (tagged.kind === "1") { tagged.payload.includes("a"); }
if (wide.tag === 2) { wide.items.find(x => x); }
