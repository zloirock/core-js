...
const fixedNs = { Base: Array };
class Fixed extends fixedNs.Base {
  static go() { return super.of(1, 2); }
}

let noneNs = { Base: Boolean };
if (c) noneNs = { Base: Number };
...
export function opaque(P) {
  class Unnameable extends P {
    static go() { return super.fromAsync([1]); }
  }
  return Unnameable.go();
}

let resolvedNs = { Base: Boolean };
if (c) resolvedNs = { Base: Array };
class InstanceKeyOverResolvedArm extends resolvedNs.Base {
  static go() { return super.flat(); }
}

let carriedNs;
switch (c) { case 1: carriedNs = { Base: Boolean }; break; default: carriedNs = { Base: Array }; }
class InstanceKeyOverCarrier extends carriedNs.Base {
  static go() { return super.at(0); }
}

class Local { static from() { return 0; } }
let localNs = { Base: Local };
if (c) localNs = { Base: Boolean };
