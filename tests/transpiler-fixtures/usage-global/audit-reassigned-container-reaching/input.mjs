...
let wBr = { b: Object };
wBr = Math.random() > 0.5 ? { b: Number } : { b: Object };
const { b: { isSafeInteger: viaBranchingWrite } } = wBr;
...
let wNu;
wNu ??= { u: Object };
const { u: { getOwnPropertyNames: viaNullishBinding } } = wNu;
let wAn = { i: Object };
wAn &&= { i: Number };
const { i: { parseInt: viaAndBinding, isExtensible: viaAndInit } } = wAn;

// NEGATIVE: a for-in head rebind yields KEYS (strings) - no enumerable container value, so
// only the literal candidate injects
let wFi = { f: Object };
for (wFi in { a: 1 }) void 0;
const { f: { getOwnPropertyDescriptors: viaForInLiteralOnly } } = wFi;
...
const { y: { asyncIterator: viaShadowClean } } = wSh;
