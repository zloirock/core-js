...
if (c1()) ({ at: m1 } = obj.list);
if (!c2()) ; else ({ flat: m2 } = obj.list);
for (let i = 0; i < 1; i++) ({ includes: m3 } = obj.list);
for (const x of [1]) ({ findLast: m4 } = obj.list);
for (const k in { a: 1 }) ({ copyWithin: m5 } = obj.list);
do ({ fill: m6 } = obj.list); while (false);
