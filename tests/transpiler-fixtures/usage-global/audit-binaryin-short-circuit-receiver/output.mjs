...
export function probes(absent) {
  ...
  return ['flat' in arr?.slice(), 'at' in arr?.slice(), 'includes' in list?.concat([]), 'entries' in arr?.slice().slice()];
}
...
export function logicalReceivers(absent) {
  const arr = absent ? null : src;
  return ['flat' in (arr && src.slice()), 'at' in (arr || src.slice()), 'includes' in (arr ?? src.slice()), 'entries' in ([1] && src.slice())];
}
...
export function composedWraps(absent) {
  const arr = absent ? null : src;
  return ['flat' in arr?.slice(globalThis.window?.self.Math.trunc(0.5)), (() => 'at' in arr?.slice().concat([4]))(), ['flat' in arr?.slice(), 'at' in arr?.slice()], 'flat' in ('at' in arr?.slice() ? src.slice() : src)];
}