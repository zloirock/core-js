...
const WA = [Array];
const WN = { a: { b: Array } };
for (const { from } of [W.w]) out.push(typeof from);
for (const { from } of [WA[0]]) out.push(typeof from);
for (const { from } of [WN.a.b]) out.push(typeof from);
for (const { w: { from } } of [W]) out.push(typeof from);
// NEGATIVE: a slot the source WRITES holds whatever was written, so the walk owes the live read
const written = { w: Array };
written.w = Map;
for (const { from } of [written.w]) out.push(typeof from);
...
const throughHead = { w: Array };
for (const { from } of [throughHead.w]) {
  throughHead.w = Map;
...
const reentered = { w: Array };
for (let round = 0; round < 2; round += 1) {
  for (const { from } of [reentered.w]) {
    reentered.w = Map;
    out.push(typeof from);
  }
}
...
const observed = { get w() { out.push('got'); return Array; } };
for (const { from } of [observed.w]) out.push(typeof from);
export { out };
