...
function makeExplicit<T = number[]>(): T { return [] as any; }
...
function fromUnionParam<T = string>(x: T | null): T { return x as any; }
fromUnionParam(new Date()).includes(1);

// channel: transitive dependent default (U = T) with an opaque earlier param
function makeTransitive<T = number[], U = T>(x: T): U { return x as any; }
...
type Wrap<T> = { v: T; };
function wrapIt<T = string>(x: T): Wrap<T> { return { v: x } as any; }
wrapIt('abc').v.at(0);
wrapIt([1, 2]).v.includes(1);
function wrapOpaque<T = string>(x: T | null): Wrap<T> { return { v: x } as any; }
