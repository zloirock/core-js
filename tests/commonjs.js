...
for (const PATH of ['../packages/core-js', '../packages/core-js-pure']) {
  ...
  const Map = require(`${ PATH }/fn/map`);
  const Set = require(`${ PATH }/fn/set`);
  const WeakMap = require(`${ PATH }/fn/weak-map`);
  const WeakSet = require(`${ PATH }/fn/weak-set`);
  ok(new Map([[1, 2], [3, 4]]).size === 2);
  ok(new Set([1, 2, 3, 2, 1]).size === 3);
  ok(new WeakMap([[O = {}, 42]]).get(O) === 42);
  ok(new WeakSet([O = {}]).has(O));
  ok(require(`${ PATH }/fn/map/of`)([1, 2], [3, 4]) instanceof Map);
  ok(require(`${ PATH }/fn/set/of`)(1, 2, 3, 2, 1) instanceof Set);
  ok(require(`${ PATH }/fn/weak-map/of`)([{}, 1], [[], 2]) instanceof WeakMap);
  ok(require(`${ PATH }/fn/weak-set/of`)({}, []) instanceof WeakSet);
  ok(require(`${ PATH }/fn/map/from`)([[1, 2], [3, 4]]) instanceof Map);
  ok(require(`${ PATH }/fn/set/from`)([1, 2, 3, 2, 1]) instanceof Set);
