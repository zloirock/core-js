...
| `transpiler-perf/` | `npm run test-transpiler-perf`, `test-transpiler-perf-smoke` | transpiler perf gates and complexity-class discriminators |
...
For routine agent perf checks, including the final check of work unrelated to performance, use `npm run test-transpiler-perf-smoke` (1 measured pass, no warmup), also used by the local composite. The bare `test-transpiler-perf` command defaults to warmup and 3 measured passes, as used in CI and for performance investigations and before/after timing conclusions; see `transpiler-perf/AGENTS.md`.
...
- A runner's summary is a claim about a denominator, so it carries both halves: an identity saying every collected case left through exactly one counter, and a FLOOR under the count that actually compared. Read the counter where the collecting ENDS, never mid-file: a summary computed above later call sites reports them as text and gates on none of them, and that run is green with a smaller denominator nobody reads. The identity alone is scale-free - it holds just as well when the whole corpus left through a declared skip, which is what a comparator that stopped comparing produces, and that run is byte-identical to a clean one but faster. A run that WRITES a baseline compares nothing and must fail rather than report a pass
