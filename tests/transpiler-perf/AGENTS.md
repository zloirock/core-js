...
The declined-pattern mirror also has a deterministic resolver-call budget in `tests/polyfill-provider/mirror-decline-complexity.mjs`: repeated declines must stay linear in the pattern width. Its counter can distinguish the old quadratic path without machine-dependent timing.
