...
Complexity-class gates for both emitters (unplugin in both engines: the `unplugin` rows run the text engine, `unplugin-ast` the AST engine, sharing the text rows' complexity-class bounds unless a case overrides them). The corpus is real packages, pinned in this directory's `package.json` - large single-scope bundles, and sets of modules from tiny to mid-sized - plus synthetic sources that push a single analysis to its worst shape.
