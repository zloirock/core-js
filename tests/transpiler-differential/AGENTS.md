...
- `harness.mjs` - runs one snippet through the pure legs (native, both emitters, the AST engine, the print-through) and compares them
...
reproducible disagreement marks the group DRIFTED: the address folds the prefix's SOURCES, but the
shared realm is enriched by the prefix's transformed OUTPUTS (a plugin edit moves those without
moving any address), and a snippet may also observe the realm rather than itself (a key count over a
shared object, a builtin a neighbour rewrote). A drifted group is never trusted cached - the shard
replays the whole snippet live, believes only the live verdict, and the fresh values overwrite the
stale ones; the run's summary counts them as `drifted`. A real product divergence still fails - on
live values. An UNreproducible disagreement is a snippet that is not a function of its code alone:
it is evicted and never cached. To re-check one result, re-run its chunk (`DIFF_SHARD="k/N"`) or the
run itself - both re-compute what you doubt.
...
- **Pick the leg by what it can SEE**, and set the family's flags from that, never from what looks thorough:
  ...
  - the AST leg is the only one that sees the AST engine's own REWRITE, and it is a different body from the text leg's: nothing the text leg runs answers for it. Its import set answers to the TEXT leg rather than to babel - the detection is shared, so a difference there is emission, not detection, and the import-parity oracle above already holds that detection to babel. The full-env run is where a wrong rewrite lands (a dropped effect, a receiver read twice, a guard that flipped a branch); the stripped one asks the missed-injection and wrong-root question of THAT body, which the text leg's strip run cannot answer for a body it never produced. A row flagged `textLags: true` is the accepted exception: the TEXT emitter is known to lag a fixed babel there (a claim it does not inject, tolerated because that layer is slated for deletion), so babel-vs-text parity relaxes to "text is a subset of babel" and the AST leg's import set anchors to BABEL instead
