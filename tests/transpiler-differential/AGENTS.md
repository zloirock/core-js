...
- `rig-aliases.mjs` - live `self` / `window` slots for the proxy-hop corpora, installed from OUTSIDE the snippet: an in-module `globalThis.self = ...` is a slot mutation under the mutated-statics canon and would switch off the very substitutions those corpora compare
- `serialize.mjs` - the runtime comparison's serializer, apart from `harness.mjs` on purpose: the stripped worker imports it and must NOT pull in `@babel/core`, which uses the builtins that worker strips
...
Results are remembered under `~/.cache/core-js-differential/`, one entry per (snippet, evaluation), kept in a group under the snippet's name; a changed result overwrites its own entry, so nothing accumulates. An entry may only answer a question identical to the one it answered before, so its address names everything the answer depends on:
...
The cache decides no verdict - it skips work whose answer is known, and the AUDIT keeps that honest by re-computing a rotating slice each run and comparing. A silent audit is the normal case. A reproducible disagreement marks the group DRIFTED: the address folds the prefix's SOURCES, but the shared realm is enriched by the prefix's transformed OUTPUTS (a plugin edit moves those without moving any address), and a snippet may also observe the realm rather than itself. A drifted group is never trusted cached - the shard replays the whole snippet live, believes only the live verdict, and the fresh values overwrite the stale ones; the run's summary counts them as `drifted`. A real product divergence still fails - on live values. An UNreproducible disagreement is a snippet that is not a function of its code alone: it is evicted and never cached. To re-check one result, re-run its chunk (`DIFF_SHARD="k/N"`) or the run itself - both re-compute what you doubt.

`INVALIDATE_CACHE=1` throws everything away and re-executes the corpus. **A last resort, not a habit**: it makes the audit's message disappear and leaves the hole for the next person. Use it only when a run is suspect in a way none of the four address items can express, and say which way.
...
- A PRELUDE line that is not a declaration disarms the usage-global leg for the WHOLE run: one bare expression statement there takes `armed` to zero while every other number stays put, so the leg's oracle goes silent without a single failure to show for it. Read the `Global leg: N armed of M checked` line on every corpus edit - it is the only place that silence is visible
- A cell where BOTH emitters leave the claim native is a gap in the product, not a corpus failure to hide: the row names those hosts in `skipHosts` with the reason written beside it, so what the corpus does not cover stays enumerable from the corpus itself and re-adding a host is the gap's own fail-before
...
- **Pick the leg by what it can SEE**, and set the family's flags from that, never from what looks thorough:
  ...
  - each emitter's REWRITE is its own body: nothing the other leg runs answers for it. The full-env run is where a wrong rewrite lands (a dropped effect, a receiver read twice, a guard that flipped a branch); the stripped one asks the missed-injection and wrong-root question of each body separately
