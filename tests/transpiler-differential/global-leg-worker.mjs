...
if (workerData.nativeAlias !== false) Object.defineProperty(globalThis, 'self', { value: globalThis, configurable: true, enumerable: false, writable: true });
...
if (workerData.strip !== false) runInThisContext(buildStripScript(workerData.stripGlobals ?? ['Iterator', 'globalThis']));

let result;
try {
  // The transpiler's Script host is CommonJS: program vars belong to the wrapper.
  // A fresh worker keeps both its module cache and any slot writes out of other cases.
  const mod = workerData.sourceType === 'script'
    ? createRequire(workerData.file)(workerData.file)
