...
deepStrictEqual(all.list.map(it => `${ it.engine } ${ it.version }`), [
...
deepStrictEqual(declared.list.map(it => `${ it.engine } ${ it.version }`),
  ['chrome 90', 'chrome 95', 'chrome 110'], 'targets-1 #1');
