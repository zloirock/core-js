...
QUnit.test('Promise.allSettledKeyed', assert => {
  ...
  assert.isFunction(allSettledKeyed);
  assert.arity(allSettledKeyed, 1);
});
...
QUnit.test('Promise.allSettledKeyed, resolved with empty object', assert => {
  return Promise.allSettledKeyed({}).then(it => {
    assert.deepEqual(it, nullProto({}), 'resolved with a correct value');
  });
});
...
QUnit.test('Promise.allSettledKeyed, method from direct entry, without constructor context', assert => {
  return $allSettledKeyed({ a: Promise.resolve(1) }).then(it => {
    assert.deepEqual(it, nullProto({ a: { status: 'fulfilled', value: 1 } }), 'resolved with a correct value');
  });
});

QUnit.test('Promise.allSettledKeyed, method from direct entry, with null context', assert => {
  return $allSettledKeyed.call(null, { a: Promise.resolve(1) }).then(it => {
    assert.deepEqual(it, nullProto({ a: { status: 'fulfilled', value: 1 } }), 'resolved with a correct value');
  });
});
