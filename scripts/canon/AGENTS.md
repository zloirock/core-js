...
- `npm run canon -- sets` - sites walking a wrapper set (paren / chain / TS assertions) by HAND instead of through the canon set or peel that owns it. `delta` cannot see these - they add no symbol - and that is how a peel sweep leaves inline re-derivations behind. An enumeration, like `dupes`: a site that mixes a wrapper test with an identity or terminator rule is a legitimate hit to adjudicate, not a defect
