...
// module class
function toKeyVal(object){
  if(object.length<2)return Object(object[0])
  var result={};
  result[object[0]]=object[1];
  return result
}
function getProperty(src,key){
  return(getOwnPropertyDescriptor(src,key)||$Object).value
}
var classTestSuper=/xyz/.test(function(){xyz})?/\$sup(?:er)?\b/:/.*/,           // регулярка для проверки метода на наличие вызова родительского
classWrapSuper=function(obj,cont){                                              // при необходимости оборачиваем методы для простого вызова родительских
  getOwnPropertyNames(obj).each(function(key){
    if(isFunction(getProperty(obj,key))&&isFunction(cont[key])&&classTestSuper.test(obj[key])){
      var parent=cont[key],fn=obj[key];                                         // если метод не в прото, есть родительский и необходимо обернуть
      obj[key]=function(){                                                      // обертка
        var rez,tmp=this.$super;                                                // запоминаем предыдущий метод. на случай вызова другого метода с вызовом родительского из текущего
        this.$super=parent;                                                     // пишем родителя
        rez=fn.apply(this,arguments);                                           // выполняем метод
        this.$super=tmp;                                                        // возвращаем все на родину
        return rez
      }
    }
  });                                                                           // в замыкание передаем родительский метод и сам метод
  return obj
},
classProp=function(F,obj){                                                      // добавляем свойства в прототип и дефаулты
  forOwnKeys(getOwnPropertyDescriptors(obj),function(prop,key){
    if(key.charAt(0)=='$'&&key.length>1)defineProperty(F,key.slice(1),prop);    // объекты-НЕ-функции - в дефаулты
    else defineProperty((prop.value&&typeof prop.value=='object')?F.__def:F.prototype,key,prop)
  });                                                                           // примитивы и функции - в прототип
  return F
},
classInitKeys=['$init','$mix','$invoke','$def','$proto'],                       // спец. свойства объекта параметров
Class=global.Class=function(a,b){                                               // создаём "класс"
  function F(){                                                                 // создаём обертку конструктора
    var that,result,invoke;
    if(this instanceof F)that=this;                                             // если вызван как конструктор не меняем контекст
    else{
      if(isFunction(invoke=F.$invoke))return invoke.apply(this,arguments);      // если $invoke - функция - возвращаем результаты её выполнения
      that=invoke===false?this:create(F.$p)
    }                                                                           // если $invoke - false - не меняем контекст, иначе создаем объект из прототипа
    merge(that,F.__def,1);                                                      // добавляем дефаулты в инстанс
    result=F.$init.apply(that,arguments);                                       // применяем на него конструктор
    return result===Object(result)?result:that
  }                                                                             // если результат конструктора - объект - возвращаем его, нет - текущий объект
  var O   = (b?b:a)||{},i=5,                                                    // объект свойств
      ext = b?a:ClassBase,                                                      // что наследуем. если нет родителя - делаем им базовый класс
      mix = O.$mix?isArray(O.$mix)?O.$mix:[O.$mix]:[];                          // примеси
  merge(F.prototype,O.$proto,0,0,1);                                            // пишем в прототип конструктора $proto
  F.$init=isFunction(O.$init)?O.$init:function(){};                             // добавляем базовый конструктор, если не задан или он не функция - пустая функция
  F.__def=Object(O.$def);                                                       // свойства что мержатся с инстансом при вызове конструктора
  F.$invoke=O.$invoke;                                                          // вызов конструктора без new
  while(i--)delete O[classInitKeys[i]];                                         // чистка лишнего
  classProp(F,O);                                                               // добавляем свойства в прототип и дефаулты из основного объекта
  ext===ClassBase||ext.prototype instanceof ClassBase||mix.unshift(ClassBase);  // если родитель класса не базовый класс или его потомок - добавляем базовый класс как примесь 
  classWrapSuper(F,ext)                                                         // оборачиваем статику и прототип для вызова родительских методов
  classWrapSuper(F.prototype,ext.prototype);
  merge(F,ext,0,1,1);                                                           // добавляем статику родителя
  F.prototype=make(ext.prototype,F.prototype,1);                                // пишем экземпляр объекта на базе прототипа родителя в прототип потомка, читай наследование
  merge(F.__def,ext.__def,0,1);                                                 // добавляем дефаулты
  mix.forEach(function(mx){                                                     // добавляем примеси
    merge(F,mx,0,1,1);
    merge(F.prototype,mx.prototype,0,1,1);
    merge(F.__def,mx.__def,0,1)
  });
  F.$ext=ext;                                                                   // ссылка на родителя
  F.$parent=ext.prototype;                                                      // ссылка на прототип родителя
  F.$p=F.prototype;                                                             // сокращение прототипа
  return F
},                                                                              // возвращаем конструктор
classAndProtoProps={                                                            // методы, что идут и в класс, и в прототип инстанса
  $super:function(){
    throw Error('Method has no parent')
  },                                                                            // вызов родительского метода. вернее, заглушка
  $sup:function(arg){
    return this.$super.apply(this,arg)
  },                                                                            // вызов родительского метода с массивом аргументов
  $bind:function(fn){
    return (isFunction(fn)?fn:this[fn]).bind(this)
  }
},                                                                              // возвращаем метод, прикрепленный к объекту. Аргумент - функция или имя метода объекта
ClassBase=Class.Base=extendBuiltInObject(extendBuiltInObject(function(){},classAndProtoProps),{// базовый класс
  $extend:methodize.call(Class),                                                // наследуем класс от данного
  $prop:function(){return classProp(this,toKeyVal(arguments))},                 // добавляем свойства к классу
  $proto:function(){                                                            // добавляем свойства в прототип
    merge(this.$p,classWrapSuper(toKeyVal(arguments),getPrototypeOf(this.$p)),0,0,1);
    return this
  },
  $def:function(){                                                              // добавляем свойства в конструктор
    merge(this.__def,toKeyVal(arguments));
    return this
  },
  $stat:function(){                                                             // добавляем статические свойства к классу
    return merge(this,classWrapSuper(toKeyVal(arguments),this.$ext),0,0,1)
  }
});
Class[prototype]=extendBuiltInObject(extendBuiltInObject(ClassBase[prototype],classAndProtoProps),{// прототип базового класса + для instanceof Class + mx
  $clone:methodize.call(clone),                                                 // клонируем
  $extend:function(obj,deep,desc){                                              // клонируем и дополняем
    return merge(clone(this,deep,desc),obj,deep,desc)
  }
});
function classEventsOn(once){
  return Function('e,f,s','return ((this.__events||(this.__events={}))[e]||(this.__events[e]=[])).push([f,s,'+once+']),this')
}
Class.Events=Class({                                                            // менеджер событий
  run:function(event,args){                                                     // запускаем событие
    for(var cur,i=0,e=(this.__events||$Object)[event]||$Array;i<e.length;++i){
      (cur=e[i])[0].apply(cur[1]||this,args);
      cur[2]&&e.splice(i--,1)
    }                                                                           // если once - удаляем
    return this
  },
  on:classEventsOn(false),                                                      // биндим к событию калбэк, опционально контекст
  once:classEventsOn(true),                                                     // одноразовый калбэк
  off:function(event,fn){                                                       // снимаем калбэки
    if(this.__events){
      if(!event)this.__events={};                                               // снимаем все события с объекта
      else if(!fn)this.__events[event]=[];                                      // снимаем все калбэки с события
      else for(var e=this.__events[event]||$Array,i=0,l=e.length;i<l;i++)       // снимаем с события калбэк
        if(e[i][0]===fn)e.splice(i,1)
    }
    return this
  }
});
