...
!function(){
  ...
  $def(PROTO + FORCED * ($.ES5Object != Object), 'Array', {
  ...
  $def(PROTO, 'Array', {
    ...
    forEach: $.each = $.each || createArrayMethod(0),
  ...
  $def(PROTO, 'String', {trim: createReplacer(/^\s*([\s\S]*\S)?\s*$/, '$1')});
  
  // 20.3.3.1 / 15.9.4.4 Date.now()
  $def(STATIC, 'Date', {now: function(){
    return +new Date;
  }});
  ...
  $def(PROTO, 'Date', {toISOString: function(){
