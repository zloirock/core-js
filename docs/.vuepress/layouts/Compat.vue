...
function calcValue(env: typeof environments[number], value: boolean | string | undefined) {
  switch (value) {
    case true:
...
function calcClass(env: typeof environments[number], value: boolean | string | undefined) {
  switch (value) {
...
onMounted(async () => {
  ...
  for (const [moduleName, moduleData] of Object.entries(compatData.value)) {
    try {
      const test = ((window as any).tests as Record<Partial<typeof environments[number]>, Function | Array<Function>>)[moduleName];
      if (!test)
        moduleData["your-browser"] = "no available test";
      else if (test instanceof Array)
        moduleData["your-browser"] = test.reduce((value, testFunc) => value && !!testFunc(), true);
      else
        moduleData["your-browser"] = !!test();
    } catch {
